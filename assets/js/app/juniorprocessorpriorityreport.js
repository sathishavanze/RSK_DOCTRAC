	...
	function fetchorders(OrderUID,WorkflowModuleUID,Orderlistname,Priority)
	{
		...
		orderslist = $('#orderslist').DataTable( {
			...
			"ajax": {
				"url": base_url+"JuniorProcessorPriority_Report/fetchorders",
				"type": "POST",
	...
	// $('.tabledatepicker').on('dp.show', function() {
		...
		$('.tabledatepicker').datetimepicker({
			...
			if (value != $LastUpdated) {
				...
				$.ajax({
					type: "POST",
					dataType: 'JSON',
					url: base_url + 'JuniorProcessorPriority_Report/Update_Dynamic_Column',
	...
	$(document).off('change', '.DocumentExpiryDate').on('change', '.DocumentExpiryDate', function (e) {
		...
		$.ajax({
			...
			dataType: 'JSON',
			global: false,
			url:'JuniorProcessorPriority_Report/Update_Dynamic_Column',
			data: {'OrderUID':OrderUID,'value':value,'ColumnName':ColumnName},
			success: function(data)
			{
				console.log(data);					
	...
	$(document).on('change', '.priorityinputfield', function(event) {
		...
		var OrderUID = $(this).closest("tr").find(".OrderUID").data("orderuid");
		$LastUpdated = $(this).closest("td").find("#LastUpdated-JuniorProcessorComments");
		if (currentvalue != $LastUpdated.val()) {
			$.ajax({
				type:'POST',
				dataType: 'JSON',
				global: false,
				url:'JuniorProcessorPriority_Report/updateprioritycomments',
	...
	$(document).ready(function(){
		...
		$(document).off('click','.listorders').on('click','.listorders',function() {
			...
			var Orderlistname = 'Junior Processor Priority Report - '+reporttabtitle+' - '+title;
		...
		$(document).off('click','.filterreport').on('click','.filterreport',function()
		{
			...
			else 
			{
				...
				var url = 'JuniorProcessorPriority_Report/fetch_counts';
				var ReportName = 'Junior Processor Priority Report';
				if(reporttype == "agingbucket") {
					url = 'JuniorProcessorPriority_Report/fetch_agingcounts';
					ReportName = 'Junior Processor Priority Report - Aging Bucket';
				}else if(reporttype == "onshoreprocessor") {
					url = 'JuniorProcessorPriority_Report/fetch_onshoreprocessorcounts';					
					ReportName = 'Junior Processor Priority Report - Processor';
				}else if(reporttype == "loantype") {
					url = 'JuniorProcessorPriority_Report/fetch_loantypecounts';
					ReportName = 'Junior Processor Priority Report - Loan Type';
				}else if(reporttype == "onshorejuniorprocessor") {
					url = 'JuniorProcessorPriority_Report/fetch_onshorejuniorprocessorcounts';
					ReportName = 'Junior Processor Priority Report - Junior Processor';
				}
	...
	$(document).off('click','.excelorderlist').on('click','.excelorderlist',function(){
		...
		var filename = $('#orderlisttitle').text()+'.xlsx';
		$.ajax({
			type: "POST",
			url: base_url+'JuniorProcessorPriority_Report/WriteOrdersExcel',
			xhrFields: {
				responseType: 'blob',
			},
	...
	$(document).off('blur', '.comments_editable').on('blur', '.comments_editable', function (e) {
		...
		var OrderUID = $(this).closest("tr").find(".OrderUID").data("orderuid");
		$LastUpdateComments = $(this).closest("td").find("#LastUpdated-JuniorProcessorComments");
	...
	$(document).off('click', '.btnOrderReverse').on('click', '.btnOrderReverse', function (e) {
		...
		$.ajax({
			type:"POST",
			url : base_url + 'JuniorProcessorPriority_Report/fetch_reversepopup',
