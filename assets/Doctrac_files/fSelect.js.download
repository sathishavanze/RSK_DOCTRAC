    ...
    $.fn.fSelect = function(options) {
        ...
        fSelect.prototype = {
            ...
            resetSelect: function() {
                var settings = this.settings;
                var labelText = [];
                this.$wrap.find('select').val([]);
                this.$wrap.find('.fs-option[data-index]')
                .not('.hidden, .disabled')
                .each(function() {
                	$(this).removeClass('selected');
                });
                this.$wrap.find('.fs-option').removeClass('selected');
            ...
            //custom functions
						/**********@author praveen kumar - 11-jan-2020**********/
            setSelected: function() {
                var settings = this.settings;
                var labelText = [];
                 var $wrap = this.$wrap;
                var selectedvalue = this.$wrap.find('select').val();
                $.each(selectedvalue, function(index, val) {
                	 /* iterate through array or object */
                	 console.log(val)
                	 $wrap.find('.fs-option[data-value='+val+']').addClass('selected');
                });
