...
class AgingBucket extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('AgingBucketmodel');
	}
	...
	public function index()
	{
		$data['content'] = 'index';
		$data['Modules'] = [];
		$data['AgingHeader'] = $this->config->item('AgingBucketHeader');
		...
		if(isset($this->parameters['DefaultClientUID']) && !empty($this->parameters['DefaultClientUID'])) {
			$data['Modules'] = $this->AgingBucketmodel->getCustomer_Milestones($this->parameters['DefaultClientUID']);
		}
	...
	function fetch_agingbucketcounts()
	{
		...
		$result = $this->AgingBucketmodel->get_agingbucketcounts($post);
    ...
    //column search
		...
		$list = $this->AgingBucketmodel->getOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->AgingBucketmodel->count_all($post),
			"recordsFiltered" =>  $this->AgingBucketmodel->count_filtered($post,''),
