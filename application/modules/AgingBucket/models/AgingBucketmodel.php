...
class AgingBucketmodel extends MY_Model {
	...
	function getCustomer_Milestones($CustomerUID,$MilestoneUID='')
	{
		if (!empty($CustomerUID)) {
			$this->db->select('mWorkFlowModules.WorkflowModuleUID,mWorkFlowModules.WorkflowModuleName,mWorkFlowModules.SystemName,mMilestone.MilestoneName,mMilestone.MilestoneUID,mCustomerWorkflowModules.SLA');
			$this->db->from('mCustomerWorkflowModules');
			$this->db->join('mCustomerMilestones','mCustomerMilestones.CustomerUID = mCustomerWorkflowModules.CustomerUID AND mCustomerMilestones.ProductUID = mCustomerWorkflowModules.ProductUID AND mCustomerMilestones.WorkflowModuleUID = mCustomerWorkflowModules.WorkflowModuleUID','LEFT');
			$this->db->join('mWorkFlowModules','mCustomerWorkflowModules.WorkflowModuleUID=mWorkFlowModules.WorkflowModuleUID');
			$this->db->join('mMilestone','mCustomerMilestones.MilestoneUID = mMilestone.MilestoneUID','left');
	...
	function get_agingbucketcounts($post)
	{
		$CustomerUID = NULL;
		...
		$AgingHeader = $this->config->item('AgingBucketHeader');
			...
			foreach ($Customerworkflows as $Customerworkflow) {
				...
				foreach ($AgingHeader as $AgingHeaderkey => $AgingHeadervalue) {
					...
					switch ($AgingHeaderkey) {
						...
						$this->db->select("GROUP_CONCAT(DISTINCT CASE WHEN TIMESTAMPDIFF(DAY, (TW_" .$Customerworkflow->SystemName.".EntryDatetime),NOW()) >= 0 AND TIMESTAMPDIFF(DAY, (TW_" .$Customerworkflow->SystemName.".EntryDatetime),NOW()) <= 30 ".$CASEWHERE." THEN  TW_" .$Customerworkflow->SystemName.".OrderUID ELSE NULL END) AS ".$AgingHeaderkey.$Customerworkflow->SystemName."");
						break;
				...
				$this->db->select("GROUP_CONCAT(DISTINCT 
					...
					(TIMESTAMPDIFF(DAY, (TW_" .$Customerworkflow->SystemName.".EntryDatetime),NOW()) >= 0 AND TIMESTAMPDIFF(DAY, (TW_" .$Customerworkflow->SystemName.".EntryDatetime),NOW()) <= 30) 
