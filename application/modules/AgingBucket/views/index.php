...
<div class="card mt-10" id="agingcount-view">
	...
	<div class="card-header card-header-danger card-header-icon">
		<div class="card-icon">
			...
			<h4 class="card-title">Aging Bucket</h4>
		</div>
		<div class="row">
			<div class="col-md-6">
				<!-- <h4 class="card-title">Aging Bucket</h4> -->
		...
		<table class="table table-hover" id="aging-data" style="">
			<thead>
				<tr>
					...
					<th class="text-center no-sort"><b>Loan Stages</b></th>
					<?php foreach ($Modules as $key => $value) { 	?>
						<th class="no-sort"><?php echo $value->WorkflowModuleName; ?></th>
					<?php } ?> 
				 ...
				 ?>
					...
					<tr class="text-center">
						...
						<?php foreach ($Modules as $key => $value) { 	?>
							...
							<td class="text-center <?php echo $bold_cls;?>"><a href="javascript:void(0);" class="text-primary listorders" data-count="<?php echo $Agingkey.$value->SystemName; ?>"  data-orderid=""  title="<?php echo $value->WorkflowModuleName.' ('.$Header.')'; ?>" data-workflowmoduleuid="<?php echo $value->WorkflowModuleUID; ?>">0</a></td>
						<?php } ?> 
					</tr>
				<?php } ?> 
...
<script type="text/javascript">
	...
	function fetch_agingcounts(data) {
		...
		return new Promise(function (resolve, reject) {  
			SendAsyncAjaxRequest('POST', 'AgingBucket/fetch_agingbucketcounts', data, 'json', true, true, function () {
				...
				agingdatatable = fnaging_datatable();
				agingdatatable.draw();
				resolve('success');
	...
	function fetchorders(OrderUID,WorkflowModuleUID,Orderlistname)
	{
		...
		orderslist = $('#orderslist').DataTable( {
			...
			"ajax": {
				"url": "<?php echo base_url('AgingBucket/fetchorders')?>",
				"type": "POST",
	...
	$(document).ready(function(){
		...
		$(document).off('click','.listorders').on('click','.listorders',function() {
			...
			var Orderlistname = 'Aging Bucket - '+title;
	...
	$(document).off('click','.excelorderlist').on('click','.excelorderlist',function(){
		...
		var filename = 'Aging Bucket - '+$('#orderlisttitle').text()+'.xlsx';
		$.ajax({
			type: "POST",
			url: '<?php echo base_url();?>AgingBucket/WriteOrdersExcel',
			xhrFields: {
				responseType: 'blob',
			},
