...
class CondoQR_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('CondoQR_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['CondoQR'];
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->CondoQR_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['CondoQR']);
	...
	function neworders_ajax_list(){
		...
		$list = $this->CondoQR_Orders_Model->CondoQROrders($post,'');

		$no = $post['start'];
		$CondoQRlist = [];

		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] 		= "CondoQR/index/";
		$Mischallenous['AssignButtonClass'] = "CondoQRPickNewOrder";
		...
		{
			$CondoQRlist 				= 	$DynamicColumns['orderslist'];
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="CondoQR/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm CondoQRPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$CondoQRlist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->CondoQR_Orders_Model->count_all(),
			"recordsFiltered" =>  $this->CondoQR_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
        ...
        //column order
		$list = $this->CondoQR_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm CondoQRPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->CondoQR_Orders_Model->inprogress_count_all(),
			"recordsFiltered" =>  $this->CondoQR_Orders_Model->inprogress_count_filtered($post),
	...
	function myorders_ajax_list()
	{
		...
		$list = $this->CondoQR_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->CondoQR_Orders_Model->myorders_count_all(),
			"recordsFiltered" =>  $this->CondoQR_Orders_Model->myorders_count_filtered($post),
	...
	function parkingorders_ajax_list()
	{
		...
		$list = $this->CondoQR_Orders_Model->parkingorders($post,'');
		$no = $post['start'];
		$myorderslist = [];
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				$Action = '<a href="CondoQR/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs CondoQRPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}	
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->CondoQR_Orders_Model->parkingorders_count_all(),
			"recordsFiltered" =>  $this->CondoQR_Orders_Model->parkingorders_count_filtered($post),
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->CondoQR_Orders_Model->WorkInProgressOrders($post,'');
			$data = [];
		...
		{
			$list2 = $this->CondoQR_Orders_Model->MyOrders($post,'');
			$data = [];
			$data[] = array('Order No','Loan No','Client','Loan Type','Milestone','Current Status','State','Aging','Due Date Time','Last Modified Date Time');
		...
		{
			$list3 = $this->CondoQR_Orders_Model->parkingorders($post,'');
			$data = [];
