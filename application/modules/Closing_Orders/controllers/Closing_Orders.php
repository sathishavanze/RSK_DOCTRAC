...
class Closing_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('Closing_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['Closing'];
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->Closing_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
    ...
    //column search
        ...
        */
		...
		$list = $this->Closing_Orders_Model->ClosingOrders($post,'');
		...
		/**
			...
			*/
		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] = "Closing/index/";
		$Mischallenous['AssignButtonClass'] = "ClosingPickNewOrder";
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="Closing/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm ClosingPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$myorderslist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Closing_Orders_Model->count_all($post),
			"recordsFiltered" =>  $this->Closing_Orders_Model->count_filtered($post,''),
    ...
    //column order
        ...
        */
		...
		$list = $this->Closing_Orders_Model->WorkInProgressOrders($post,'');
...
*/
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs ClosingPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
		    }
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Closing_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->Closing_Orders_Model->inprogress_count_filtered($post),
    ...
    //column search
        ...
        */
		...
		$list = $this->Closing_Orders_Model->MyOrders($post,'');
...
*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Closing_Orders_Model->myorders_count_all($post),
			"recordsFiltered" =>  $this->Closing_Orders_Model->myorders_count_filtered($post),
    ...
    //get_post_input_data
        ...
        */
		...
		$list = $this->Closing_Orders_Model->parkingorders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Closing_Orders_Model->parkingorders_count_all($post),
			"recordsFiltered" =>  $this->Closing_Orders_Model->parkingorders_count_filtered($post),
			"data" => $data['myorderslist'],
		);
...
function Expiredorders_ajax_list(){
    ...
    //column search
        ...
        */
		...
		$list = $this->Closing_Orders_Model->ExpiredOrders($post,'');
        ...
        */
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Closing_Orders_Model->Expiredcount_all($post),
			"recordsFiltered" =>  $this->Closing_Orders_Model->Expiredcount_filtered($post,''),
	...
	function GetNextOrder()
	{
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST') 
		{
			...
			if ($this->form_validation->run() == true)
			{
				...
				$existingarray = [];
				$ExistingOrders = $this->Closing_Orders_Model->CheckExistingOrders($this->loggedid,$ProjectUID,$Workflow);
				...
				else
				{
					...
					$data = $this->Closing_Orders_Model->AssignOrders($this->loggedid,$ProjectUID,$Workflow);
	...
	function PickExistingOrderCheck()
	{
		...
		$ProjectUID = $this->input->post('ProjectUID');
		$result = $this->Closing_Orders_Model->PickExistingOrderCheck($OrderUID);
		if($result['Status'] == 1)
		{
		...
		{
			$value = $this->Closing_Orders_Model->OrderAssign($OrderUID,$ProjectUID);
			if($value)
			{
    ...
    //column search
        ...
        */
		...
		$list = $this->Closing_Orders_Model->KickBackClosingOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Closing_Orders_Model->KickBackcount_all($post),
			"recordsFiltered" =>  $this->Closing_Orders_Model->KickBackcount_filtered($post,''),
			"data" => $data['myorderslist'],
		);
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->Closing_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->Closing_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->Closing_Orders_Model->parkingorders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
