...
<div class="card mt-40" id="Exceptionorders">
  ...
  <div class="card-body">
    <ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#ProductionReportReport" role="tablist">
          Daily Production Orders
          <span class="badge badge-pill badge-primary" id="completed-count" style="background-color: #fff;color: #000;"><?php echo $this->ProductionReportmodel->count_all(); ?></span>
    ...
    <?php //$this->load->view('common/advancesearch'); ?>
      ...
      <div class="material-datatables">
        ...
        <table class="table table-hover table-striped" id="ProductionReportReport">
            <thead>
               <tr>
                <th>Agent ID</th>
                <th>Agent Name</th>
...
<script type="text/javascript">
          var ProductionReportReport = false;
        $(function() {
          $("select.select2picker").select2({
            //tags: false,
            theme: "bootstrap",
          });
          $('#ProductionReportReport').DataTable().destroy();
        });
...
dailyproductionreports('false')
    function dailyproductionreports(formdata)
    {
          ProductionReportReport = $('#ProductionReportReport').DataTable( {
            scrollX:        true,
            scrollCollapse: true,
            fixedHeader: false,
            scrollY: '100vh',
          ...
          "ajax": {
            "url": "<?php echo base_url('ProductionReport/DailyProductionReport')?>",
            "type": "POST",
          ...
          {               
                ...
                if((ProjectUID == '') && (CustomerUID == '') && (FromDate == '') && (ToDate == '') && (ProductUID == ''))
                {
                 ...
                 dailyproductionreports(formData);
...
$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
  ...
  $.ajax({
     type: "POST",
     url: '<?php echo base_url();?>ProductionReport/WriteExcel',
          xhrFields: {
      responseType: 'blob',
    },
     data: {'formData':formData},
    ...
    {
        var filename = "ProductionReportOrders.csv";
        if (typeof window.chrome !== 'undefined') {
            //Chrome version
            var link = document.createElement('a');
            link.href = window.URL.createObjectURL(data);
            link.download = "ProductionReportOrders.csv";
            link.click();
...
$(document).off('click', '.ExportDocument').on('click', '.ExportDocument', function (e) {  
  ...
  SwalConfirmExport(OrderUID, OrderNumber, currentrow, ProductionReportReport, LoanNumber);
})
