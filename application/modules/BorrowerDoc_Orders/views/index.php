...
<div class="card mt-20">
	...
	<div class="card-body" id="filter-bar">
		...
		<ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
			<li class="nav-item BorrowerDocQueue">
				<a class="nav-link" data-toggle="tab" href="#orderslist" role="tablist">
					New Orders
					<span class="badge badge-pill badge-primary" id="newdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->BorrowerDoc_Orders_Model->count_all(); ?></span>
			...
			if (in_array($this->RoleType, $this->config->item('Internal Roles'))) {
				...
				<li class="nav-item BorrowerDocQueue">
					<a class="nav-link" data-toggle="tab" href="#workinprogresslist" role="tablist">
						Assigned Orders
						<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->BorrowerDoc_Orders_Model->inprogress_count_all(); ?></span>
					</a>
				</li>
				<li class="nav-item BorrowerDocQueue">
					<a class="nav-link " data-toggle="tab" href="#myorderslist" role="tablist">
						My Orders
						<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->BorrowerDoc_Orders_Model->myorders_count_all(); ?></span>
					</a>
				</li>
			...
			<?php if(!empty($IsParking)){?>
				<li class="nav-item">
					<a class="nav-link " data-toggle="tab" href="#parkingorderslist" role="tablist">
						Parking Orders
						<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->BorrowerDoc_Orders_Model->parkingorders_count_all(); ?></span>
					</a>
				</li>
			<?php } ?>
				<!-- Completed Orders -->
		...
		<div class="tab-content tab-space customtabpane">
			...
			$viewdata['NewOrdersTableName'] = "tblBorrowerDoc";
...
<script type="text/javascript">
	var tblBorrowerDoc = false;
	$(function() {
		$("select.select2picker").select2({
			//tags: false,
			theme: "bootstrap",
		});
		$('#tblBorrowerDoc').DataTable().destroy();
	});
...
$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
	...
	if (filter == 'completedorderslist') {
		...
		action_url = '<?php echo base_url();?>BorrowerDoc_Orders/WriteExcel';
	}
		...
		success: function(data)
		{
			...
			*/
			...
			if (typeof window.chrome !== 'undefined') {
				...
				link.download = "Borrower Doc Orders "+ filename;
				link.click();
...
function initialize(formData)
{
	...
	tblBorrowerDoc = $('#tblBorrowerDoc').DataTable( {
		scrollX:        true,
		scrollCollapse: true,
		fixedHeader: false,
		scrollY: '100vh',
		...
		"ajax": {
			"url": "<?php echo base_url('BorrowerDoc_Orders/BorrowerDocorders_ajax_list')?>",
			"type": "POST",
			"data" : {'formData':formData} 
		},
		"columnDefs": [ {
...
function workinprogressinitialize(formData){
	...
	<?php if (in_array($this->RoleType, $this->config->item('Internal Roles'))) { ?>
		...
		workingprogresstable = $('#workingprogresstable').DataTable( {
			...
			// Load data for the table's content from an Ajax source
			"ajax": {
				"url": "<?php echo base_url('BorrowerDoc_Orders/workinginprogress_ajax_list')?>",
				"type": "POST",
				"data" : {'formData':formData} 
			},
			"columnDefs": [ {
...
function myordersinitialize(formData){
	...
	<?php if (in_array($this->RoleType, $this->config->item('Internal Roles'))) { ?>
		...
		myorderstable = $('#myorderstable').DataTable( {
			...
			// Load data for the table's content from an Ajax source
			"ajax": {
				"url": "<?php echo base_url('BorrowerDoc_Orders/myorders_ajax_list')?>",
				"type": "POST",
				"data" : {'formData':formData} 
			},
			"columnDefs": [ {
...
function parkingordersinitialize(formData){
	...
	<?php if (in_array($this->RoleType, $this->config->item('Internal Roles'))) { ?>
		...
		parkingorderstable = $('#parkingorderstable').DataTable( {
			...
			// Load data for the table's content from an Ajax source
			"ajax": {
				"url": "<?php echo base_url('BorrowerDoc_Orders/parkingorders_ajax_list')?>",
				"type": "POST",
				"data" : {'formData':formData} 
			},
			"columnDefs": [ {
...
$(document).off('click','.BorrowerDocPickNewOrder').on('click','.BorrowerDocPickNewOrder',function(){
	...
	.done(function(response) {
		...
		if(response.validation_error == 2)
		{
			...
			setTimeout(function(){ 
				...
				triggerpage('BorrowerDoc/index/'+OrderUID);
