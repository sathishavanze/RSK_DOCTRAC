...
class BorrowerDoc_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('BorrowerDoc_Orders_Model');
	}
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->BorrowerDoc_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['BorrowerDoc']);
		$this->load->view($this->input->is_ajax_request() ? $data['content'] : 'page', $data);
	}

	function BorrowerDocorders_ajax_list()
	{
		//Advanced Search
    ...
    //column search
        ...
        //column order
		$list = $this->BorrowerDoc_Orders_Model->BorrowerDocorders($post,'');
...
*/
			...
			{
				$Action = '<a href="BorrowerDoc/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i></a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm BorrowerDocPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i></button>';
			}
			$row[] = $Action;
			$myorderslist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->BorrowerDoc_Orders_Model->count_all(),
			"recordsFiltered" =>  $this->BorrowerDoc_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
		...
		$list = $this->BorrowerDoc_Orders_Model->WorkInProgressOrders($post,'');
...
*/
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs BorrowerDocPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i></button>';
			}
			$row[] = $Action;
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->BorrowerDoc_Orders_Model->inprogress_count_all(),
			"recordsFiltered" =>  $this->BorrowerDoc_Orders_Model->inprogress_count_filtered($post),
    ...
    //column search
		$post['column_search'] = array('tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
        //column order
		$list = $this->BorrowerDoc_Orders_Model->MyOrders($post,'');
...
*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->BorrowerDoc_Orders_Model->myorders_count_all(),
			"recordsFiltered" =>  $this->BorrowerDoc_Orders_Model->myorders_count_filtered($post),
    ...
    //column search
		...
		$list = $this->BorrowerDoc_Orders_Model->parkingorders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->BorrowerDoc_Orders_Model->parkingorders_count_all(),
			"recordsFiltered" =>  $this->BorrowerDoc_Orders_Model->parkingorders_count_filtered($post),
			"data" => $data['myorderslist'],
		);
	...
	function GetNextOrder()
	{
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST') 
		{
			...
			if ($this->form_validation->run() == true)
			{
				...
				$existingarray = [];
				$ExistingOrders = $this->BorrowerDoc_Orders_Model->CheckExistingOrders($this->loggedid,$ProjectUID,$Workflow);
				...
				else
				{
					...
					$data = $this->BorrowerDoc_Orders_Model->AssignOrders($this->loggedid,$ProjectUID,$Workflow);
	...
	function PickExistingOrderCheck()
	{
		...
		$ProjectUID = $this->input->post('ProjectUID');
		$result = $this->BorrowerDoc_Orders_Model->PickExistingOrderCheck($OrderUID);
		if($result['Status'] == 1)
		{
		...
		{
			$value = $this->BorrowerDoc_Orders_Model->OrderAssign($OrderUID,$ProjectUID);
			if($value)
			{
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->BorrowerDoc_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->BorrowerDoc_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->BorrowerDoc_Orders_Model->parkingorders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
			*/
			...
			for ($i=0; $i < sizeof($list3); $i++) { 
				$data[] = array($list3[$i]->OrderNumber,$list3[$i]->CustomerName, $list3[$i]->LoanNumber,$list3[$i]->LoanType,$list3[$i]->MilestoneName,$list3[$i]->StatusName,$list3[$i]->PropertyStateCode,$list3[$i]->AssignedTo,site_datetimeformat($list3[$i]->AssignedDateTime),site_datetimeaging($list3[$i]->EntryDatetime),site_datetimeformat($list3[$i]->DueDateTime),site_datetimeformat($list3[$i]->LastModifiedDateTime));				
			}
