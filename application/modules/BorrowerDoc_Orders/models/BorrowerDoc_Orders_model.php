...
class BorrowerDoc_Orders_model extends MY_Model {
	...
	{
		$parkingenabled = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['BorrowerDoc']);

		$this->db->select("1");


		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetBorrowerDocQueue();

		//filter order when parking disabled
		if(empty($parkingenabled)) {
			$this->db->where("NOT EXISTS (SELECT 1 FROM tOrderParking WHERE tOrderParking.OrderUID = tOrders.OrderUID AND tOrderParking.WorkflowModuleUID='".$this->config->item('Workflows')['BorrowerDoc']."' AND tOrderParking.IsCleared = 0 )",NULL,FALSE);
		}
  ...
  // MyOrders
	function count_all()
	{
		...
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetBorrowerDocQueue();
		$this->db->where("NOT EXISTS (SELECT 1 FROM tOrderParking WHERE OrderUID = tOrders.OrderUID AND WorkflowModuleUID = '".$this->config->item('Workflows')['BorrowerDoc']."' AND IsCleared = 0)",NULL,FALSE);
	...
	function BorrowerDocorders($post,$module,$global='') {
      ...
      // Datatable OrderBy
		...
		/*######### Loan Metrics ##########*/
		$MetricsOrderBy = $this->Common_Model->getMetricsDependentworkflows($this->parameters['DefaultClientUID'], $this->config->item('Workflows')['BorrowerDoc']);
  ...
  // Work in Progress
	...
	function parkingorders_count_all()
	{
		...
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetBorrowerDocQueue();
		$this->db->join("tOrderParking","tOrderParking.OrderUID=tOrders.OrderUID and tOrderParking.WorkflowModuleUID='".$this->config->item('Workflows')['BorrowerDoc']."' AND tOrderParking.IsCleared = 0 ");
