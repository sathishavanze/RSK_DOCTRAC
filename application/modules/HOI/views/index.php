					...
					<form action="#"  name="orderform" id="frmordersummary">
						...
						<?php $this->load->view('Ordersummary/Orderdetails'); ?>
							...
							<tbody class="addChecklist">
								<?php $question_sno =0; foreach ($DocumentTypeNameDetails as $key => $DocTypeName) {
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('input[data-ChildLable=Email]').on('change', function(){
			var email = $(this).val();
		...
		$('.parentUID').on('change', function(){
			var uid = $(this).data('uid');
			var val = $(this).val();
			var chld_lng = $('.child-'+uid).length;

			// alert(chld_lng);
			if(val != '' && chld_lng > 0){

				$.ajax({
					url: 'Documenttype/CheckChildValue',
					...
					success: function(data){
						if(data) {
							$.each(data, function(key,value) {
							  var selector = 'input[data-ChildLable='+key+']';
							  $(selector).val(value);
							  $(selector).focus();
							}); 
						}
