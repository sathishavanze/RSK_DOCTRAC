...
class PayOffOrdersmodel extends MY_Model {
  ...
  function total_count()
  {
    $this->db->select("1");
      ...
      /*^^^^^ Get PayOff Update Orders Query ^^^^^*/
      $this->Common_Model->GetPayOffUpdateOrders();
...
$this->Common_Model->FilterByProjectUser($this->RoleUID,$this->loggedid);     
  ...
  function PayOffOrders($post,$global='') {
      $this->Common_Model->DynamicColumnsCommonQuery($WorkflowModuleUID = '',TRUE);
    ...
    function GetPayOffOrdersExcelRecords($post)
    {
      $this->Common_Model->DynamicColumnsCommonQuery($WorkflowModuleUID = '',TRUE);
