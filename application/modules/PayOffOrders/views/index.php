...
<!--     <div class="row">
      <div class="col-md-6">
        <h4 class="card-title">PayOffOrders</h4>
      </div>
    </div> -->
  </div>
  ...
  <div class="card-body">
    <ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#PayOffOrders" role="tablist">
          PayOff Orders
          <span class="badge badge-pill badge-primary" id="PayOffOrders-count" style="background-color: #fff;color: #000;"><?php echo $this->PayOffOrdersmodel->count_all(); ?></span>
    ...
    <div  class="material-datatables" >
      ...
      <table class="table table-hover table-striped" id="PayOffOrders">
        <thead>
         <tr>
...
<script type="text/javascript">
  var PayOffOrders = false;
  $(function() {
    $("select.select2picker").select2({
      //tags: false,
      theme: "bootstrap",
    });
    $('#PayOffOrders').DataTable().destroy();
  });
  $(document).ready(function(){
    ...
    PayOffOrdersinitialize('false')
    function PayOffOrdersinitialize(formdata)
    {
      PayOffOrders = $('#PayOffOrders').DataTable( {
        scrollX:        true,
        scrollCollapse: true,
        fixedHeader: false,
        scrollY: '100vh',
        ...
        "autoWidth": true,
          ...
          "ajax": {
            "url": "<?php echo base_url('PayOffOrders/PayOffOrders_ajax_list')?>",
            "type": "POST",
    ...
    $(document).off('click','.search').on('click','.search',function()
    {
          ...
          //added Milestone,State, loan no.
              ...
              if((ProjectUID == '') && (MilestoneUID == '') && (StateUID == '') && (LoanNo == '') && (PackageUID == '') &&  (InputDocTypeUID == '') && (CustomerUID == '') && (FromDate == '') && (ToDate == '') && (ProductUID == ''))
              {
               ...
               PayOffOrdersinitialize(formData);
...
$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
  ...
  $.ajax({
   type: "POST",
   url: '<?php echo base_url();?>PayOffOrders/WriteExcel',
   xhrFields: {
    responseType: 'blob',
  },
  data: {'formData':formData},
  ...
  {
    var filename = 'PayOffOrders.xlsx';
...
$(document).off('click', '.ExportDocument').on('click', '.ExportDocument', function (e) {  
  ...
  SwalConfirmExport(OrderUID, OrderNumber, currentrow, PayOffOrders, LoanNumber);
})
