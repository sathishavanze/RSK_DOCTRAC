...
class GateKeeping_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('GateKeeping_Orders_Model');
		$this->load->model('ExceptionQueue_Orders/ExceptionQueue_Orders_model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['GateKeeping'];
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->GateKeeping_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['GateKeeping']);
	...
	function neworders_ajax_list(){
		...
		$list = $this->GateKeeping_Orders_Model->GateKeepingOrders($post,'');

		$no = $post['start'];
		$GateKeepinglist = [];

		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] 		= "GateKeeping/index/";
		$Mischallenous['AssignButtonClass'] = "GateKeepingPickNewOrder";
		...
		{
			$GateKeepinglist 				= 	$DynamicColumns['orderslist'];
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="GateKeeping/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm GateKeepingPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$GateKeepinglist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->GateKeeping_Orders_Model->count_all($post),
			"recordsFiltered" =>  $this->GateKeeping_Orders_Model->count_filtered($post,''),
	...
	*/
	function widgetGetPendingOrdersCount() {
    	...
    	$post['WorkflowModuleUID'] = $this->WorkflowModuleUID;
		...
		if (!empty($FilterCounts)) {
			...
			foreach ($FetchPendingCountWorkflows as $WorkflowModuleName => $WorkflowModuleUID) {
				...
				$PendingWorklfowsCount[$WorkflowModuleName] = $this->{$FilterCounts['ModelName']}->{$FilterCounts['FunctionName']}($post,'count_all');
			}
			$post = [];
			$post['WorkflowModuleUID'] = $this->WorkflowModuleUID;
			...
			$post['QueueUID'] = $QueueUID;
			$PendingWorklfowsCount['CompletedOrdersCount'] = $this->{$FilterCounts['ModelName']}->{$FilterCounts['FunctionName']}($post,'count_all');
		}
	...
	function workinginprogress_ajax_list()
	{
        ...
        //column order
		$list = $this->GateKeeping_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm GateKeepingPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->GateKeeping_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->GateKeeping_Orders_Model->inprogress_count_filtered($post),
	...
	function myorders_ajax_list()
	{
		...
		$list = $this->GateKeeping_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->GateKeeping_Orders_Model->myorders_count_all($post),
			"recordsFiltered" =>  $this->GateKeeping_Orders_Model->myorders_count_filtered($post),
	...
	function parkingorders_ajax_list()
	{
		...
		$list = $this->GateKeeping_Orders_Model->parkingorders($post,'');
		$no = $post['start'];
		$myorderslist = [];
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				$Action = '<a href="GateKeeping/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs GateKeepingPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}	
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->GateKeeping_Orders_Model->parkingorders_count_all($post),
			"recordsFiltered" =>  $this->GateKeeping_Orders_Model->parkingorders_count_filtered($post),
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->GateKeeping_Orders_Model->WorkInProgressOrders($post,'');
			$data = [];
		...
		{
			$list2 = $this->GateKeeping_Orders_Model->MyOrders($post,'');
			$data = [];
			$data[] = array('Order No','Loan No','Client','Loan Type','Milestone','Current Status','State','Aging','Due Date Time','Last Modified Date Time');
		...
		{
			$list3 = $this->GateKeeping_Orders_Model->parkingorders($post,'');
			$data = [];
	...
	/**
	*Function Get Re-Work Orders Ajax List 
	*@author SathishKumar <sathish.kumar@avanzegroup.com>
	*@since Tuesday 18 August 2020.
	*/
	function ReWorkOrdersAjaxList(){
		//Advanced Search
		$post['advancedsearch'] = $this->input->post('formData');
		//Advanced Search
		//get_post_input_data
		...
		$list = $this->GateKeeping_Orders_Model->WorkUpReWorkGateKeepingOrders($post,'');
		...
		$Mischallenous['AssignButtonClass'] = "GateKeepingPickNewOrder";
		$Mischallenous['QueueColumns'] = $QueueColumns;
		$Mischallenous['IsReworkOrders'] = true;
		$DynamicColumns = $this->Common_Model->getDynamicQueueColumns($list, $this->WorkflowModuleUID, $Mischallenous);
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->GateKeeping_Orders_Model->WorkUpReWorkGateKeepingOrders_CountAll($post),
			"recordsFiltered" =>  $this->GateKeeping_Orders_Model->WorkUpReWorkGateKeepingOrders_CountFiltered($post,''),
			"data" => $data['orderslist'],
		);
	...
	/**
	*Function Get Re-Work Pending Orders Ajax List 
	*@author SathishKumar <sathish.kumar@avanzegroup.com>
	*@since Monday 31 August 2020.
	*/
	function ReWorkPendingOrdersAjaxList(){
		//Advanced Search
		$post['advancedsearch'] = $this->input->post('formData');
		//Advanced Search
		//get_post_input_data
		...
		$list = $this->GateKeeping_Orders_Model->ReWorkPendingOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->GateKeeping_Orders_Model->ReWorkPendingOrders_CountAll($post),
			"recordsFiltered" =>  $this->GateKeeping_Orders_Model->ReWorkPendingOrders_CountFiltered($post,''),
			"data" => $data['orderslist'],
		);
