...
class CD_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('CD_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['CD'];
		$this->WorkflowModuleName = 'CD';
	} 
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->CD_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['CD']);
    ...
    //column search
        ...
        */
		...
		$list = $this->CD_Orders_Model->CDOrders($post,'');
		...
		{
			$CDlist 				= 	$DynamicColumns['orderslist'];
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->CD_Orders_Model->count_all(),
			"recordsFiltered" =>  $this->CD_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
  	...
  	//column order
        ...
        */
        ...
        //column order
		$list = $this->CD_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm CDPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->CD_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->CD_Orders_Model->inprogress_count_filtered($post),
    ...
    //column search
        ...
        */
		...
		$list = $this->CD_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->CD_Orders_Model->myorders_count_all(),
			"recordsFiltered" =>  $this->CD_Orders_Model->myorders_count_filtered($post),
    ...
    //column search
        ...
        */
		...
		$list = $this->CD_Orders_Model->parkingorders($post,'');
		$no = $post['start'];
		$myorderslist = [];
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				$Action = '<a href="CD/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs CDPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}	
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->CD_Orders_Model->parkingorders_count_all(),
			"recordsFiltered" =>  $this->CD_Orders_Model->parkingorders_count_filtered($post),
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->CD_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->CD_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->CD_Orders_Model->parkingorders($post,'');
