...
class Pipeline extends MY_Controller {
	...
	public function index()
	{
		...
		if(isset($this->parameters['DefaultClientUID']) && !empty($this->parameters['DefaultClientUID'])) {
			$data['Milestones'] = $this->config->item('PriorityReport_Milestones');
			$data['Workflows'] = $this->Pipeline_model->get_PipelineReport_Workflows($this->parameters['DefaultClientUID']);
		}
	...
	function fetch_counts()
	{
		...
		$result = $this->Pipeline_model->get_counts($post);
	...
	function fetch_agingcounts()
	{
		...
		$post['FromDate'] = $this->input->post('FromDate');
		$post['ToDate'] = $this->input->post('ToDate');
		$result = $this->Pipeline_model->get_pipelineagingcounts($post);
		echo json_encode(['success'=>1,'data'=>$result]);
	}
	...
	function fetch_processorcounts()
	{
		...
		$post['FromDate'] = $this->input->post('FromDate');
		$post['ToDate'] = $this->input->post('ToDate');
		$result = $this->Pipeline_model->get_priorityprocessorcounts($post);
		echo json_encode(['success'=>1,'data'=>$result]);
	}
	...
	function fetch_teamleadcounts()
	{
		...
		$post['FromDate'] = $this->input->post('FromDate');
		$post['ToDate'] = $this->input->post('ToDate');
		$result = $this->Pipeline_model->get_priorityteamleadcounts($post);
		echo json_encode(['success'=>1,'data'=>$result]);
	}
	...
	function fetch_onshoreprocessorcounts()
	{
		...
		$post['FromDate'] = $this->input->post('FromDate');
		$post['ToDate'] = $this->input->post('ToDate');
		$result = $this->Pipeline_model->get_priorityonshoreprocessorcounts($post);
		echo json_encode(['success'=>1,'data'=>$result]);
	}
	...
	function fetch_onshoreteamleadcounts()
	{
		...
		$post['FromDate'] = $this->input->post('FromDate');
		$post['ToDate'] = $this->input->post('ToDate');
		$result = $this->Pipeline_model->get_priorityonshoreteamleadcounts($post);
		echo json_encode(['success'=>1,'data'=>$result]);
	}
	...
	function fetch_loantypecounts()
	{
		...
		$post['FromDate'] = $this->input->post('FromDate');
		$post['ToDate'] = $this->input->post('ToDate');
		$result = $this->Pipeline_model->get_pipelineloantypecounts($post);
		echo json_encode(['success'=>1,'data'=>$result]);
	}
    ...
    //column search
		...
		$list = $this->Pipeline_model->getOrders($post);
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Pipeline_model->count_all($post),
			"recordsFiltered" =>  $this->Pipeline_model->count_filtered($post),
			"data" => $returnlist,
		);
	...
	function WriteOrdersExcel()
	{
		...
		foreach($list as $Order) {
			$Exceldataset = array($Order->OrderNumber,$Order->CustomerName,$Order->LoanNumber,$Order->LoanType,$Order->MilestoneName,$Order->StatusName,$Order->PropertyStateCode,$Order->Aging,site_datetimeformat($Order->DueDateTime),site_datetimeformat($Order->LastModifiedDateTime));
