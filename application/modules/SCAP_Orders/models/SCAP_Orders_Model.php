...
class SCAP_Orders_Model extends MY_Model {
	...
	{
		$parkingenabled = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['SCAP']);

		$this->db->select("1");

		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetSCAPOrders();

		//filter order when parking disabled
		if(empty($parkingenabled)) {
			$this->db->where("NOT EXISTS (SELECT 1 FROM tOrderParking WHERE tOrderParking.OrderUID = tOrders.OrderUID AND tOrderParking.WorkflowModuleUID='".$this->config->item('Workflows')['SCAP']."' AND tOrderParking.IsCleared = 0 )",NULL,FALSE);
		}
	...
	function count_all()
	{
		...
		/*^^^^^ Get SCAP Query ^^^^^*/
		$this->Common_Model->GetSCAPOrders();
		$this->db->where("NOT EXISTS (SELECT 1 FROM tOrderParking WHERE OrderUID = tOrders.OrderUID AND WorkflowModuleUID = '".$this->config->item('Workflows')['SCAP']."' AND IsCleared = 0)",NULL,FALSE);
	...
	{
		$this->Common_Model->DynamicColumnsCommonQuery($this->config->item("Workflows")["SCAP"]);
		$this->db->select("1");
	...
	function SCAPOrders($post,$module,$global='')
	{
		$this->Common_Model->DynamicColumnsCommonQuery($this->config->item("Workflows")["SCAP"]);
		...
		$MetricsOrderBy = $this->Common_Model->getMetricsDependentworkflows($this->parameters['DefaultClientUID'], $this->config->item('Workflows')['SCAP']);
	...
	function parkingorders_count_all()
	{
		...
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetSCAPOrders();
		$this->db->join("tOrderParking","tOrderParking.OrderUID=tOrders.OrderUID and tOrderParking.WorkflowModuleUID='".$this->config->item('Workflows')['SCAP']."' AND tOrderParking.IsCleared = 0 ");
