...
<div class="card mt-10" id="Exceptionorders">
    ...
    <div class="card-icon">
      ...
      <h4 class="card-title">Docs Out Pending</h4>
    </div>
<!--     <div class="row">
      <div class="col-md-6">
        <h4 class="card-title">Docs Out Pending</h4>
    ...
    <ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#review" role="tablist">
          Docs Out Pending Orders
          <!-- <span class="badge badge-pill badge-primary" id="DocsOutPending-count" style="background-color: #fff;color: #000;"><?php echo $this->DocsOutPendingordersmodel->count_all(); ?></span> -->
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    DocsOutPendinginitialize('false')
    function DocsOutPendinginitialize(formdata)
    {
      review = $('#review').DataTable( {
        ...
        "autoWidth": true,
          ...
          "ajax": {
            "url": "<?php echo base_url('DocsOutPending/DocsOutPendingorders_ajax_list')?>",
            "type": "POST",
    ...
    $(document).off('click','.search').on('click','.search',function()
    {
          ...
          //added Milestone,State, loan no.
              ...
              if((ProjectUID == '') && (MilestoneUID == '') && (StateUID == '') && (LoanNo == '') && (PackageUID == '') &&  (InputDocTypeUID == '') && (CustomerUID == '') && (FromDate == '') && (ToDate == '') && (ProductUID == ''))
              {
               ...
               DocsOutPendinginitialize(formData);
...
$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
  ...
  $.ajax({
   type: "POST",
   url: '<?php echo base_url();?>DocsOutPending/WriteExcel',
   xhrFields: {
    responseType: 'blob',
  },
  data: {'formData':formData},
  ...
  {
    var filename = "DocsOutPendingOrders.csv";
    if (typeof window.chrome !== 'undefined') {
            //Chrome version
            var link = document.createElement('a');
            link.href = window.URL.createObjectURL(data);
            link.download = "DocsOutPendingOrders.csv";
            link.click();
