...
class FinalApproval_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('FinalApproval_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['FinalApproval'];
	}
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->FinalApproval_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['FinalApproval']);
		$this->load->view($this->input->is_ajax_request() ? $data['content'] : 'page', $data);
	}

	function finalapprovalorders_ajax_list(){
		//Advanced Search
    ...
    //column order
		...
		$list = $this->FinalApproval_Orders_Model->FinalApprovalOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="FinalApproval/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i></a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm FinalApprovalPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i></button>';
			}
			$row[] = $Action;
			$prescreenlist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->FinalApproval_Orders_Model->count_all(),
			"recordsFiltered" =>  $this->FinalApproval_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
  	...
  	//column order
        ...
        //column order
		$list = $this->FinalApproval_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm FinalApprovalPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i></button>';
			}
			$row[] = $Action;
			}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->FinalApproval_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->FinalApproval_Orders_Model->inprogress_count_filtered($post),
    ...
    //column order and search
		$post['column_order'] = array('tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
		$post['column_search'] = array('tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
		$list = $this->FinalApproval_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->FinalApproval_Orders_Model->myorders_count_all(),
			"recordsFiltered" =>  $this->FinalApproval_Orders_Model->myorders_count_filtered($post),
	...
	/*********************************Parking Order Ajax list starts************************ */
...
//get_post_input_data
	...
	//column search
	$post['column_search'] = array('tOrders.OrderNumber','mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName','mStatus.StatusName','tOrders.PropertyStateCode','b.UserName','tOrderParking.Remarks','tOrderParking.Remainder','tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
	$list = $this->FinalApproval_Orders_Model->parkingorders($post,'');
	...
	foreach ($list as $myorders)
	{
		...
		if($this->loggedid == $myorders->AssignedToUserUID)
		{
			...
			$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs FinalApprovalPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i></button>';
		}
		$row[] = $Action;
		$myorderslist[] = $row;
	}
	...
	$output = array(
		"draw" => $post['draw'],
		"recordsTotal" => $this->FinalApproval_Orders_Model->parkingorders_count_all(),
		"recordsFiltered" =>  $this->FinalApproval_Orders_Model->parkingorders_count_filtered($post),
...
/**********************************Parking Order Ajax List End****************************************** */
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->FinalApproval_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->FinalApproval_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->FinalApproval_Orders_Model->parkingorders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
