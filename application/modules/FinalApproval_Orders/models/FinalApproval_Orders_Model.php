...
class FinalApproval_Orders_Model extends MY_Model {
	...
	function total_count()
	{
		...
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetFinalApprovalOrders(false);
	...
	function total_count_filtered($post)
	{

		$this->db->select("1");

		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetFinalApprovalOrders();
  ...
  // MyOrders
	...
	function FinalApprovalOrders($post,$module,$global=''){
		$this->db->select("tOrders.*, mProjectCustomer.ProjectName, tOrders.OrderUID,tOrders.LoanNumber, mStatus.StatusName,mMilestone.MilestoneName, mStatus.StatusColor ,mCustomer.CustomerName, mProjectCustomer.ProjectUID, mProducts.ProductName, tOrderAssignments.AssignedToUserUID");
