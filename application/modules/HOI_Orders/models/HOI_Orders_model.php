...
class HOI_Orders_model extends MY_Model {
	...
	function total_count()
	{
		...
		$this->db->select("1");
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetHOIQueue(false);
  ...
  // MyOrders
	...
	function HOIorders($post,$module,$global='') {
		$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
  ...
  // Work in Progress
	...
	function Hoi_Loan_Process($post){
		...
		$this->db->select('tOrderImport.*, tOrderPropertyRole.*, mUsers.UserName AS AssignedUserName, tOrderAssignments.AssignedDatetime, tResponseTrack.*');
		$this->db->select("tOrders.*,  mProjectCustomer.ProjectName, tOrders.OrderUID,mMilestone.MilestoneName, mStatus.StatusName, mStatus.StatusColor ,mCustomer.CustomerName, mProjectCustomer.ProjectUID, mProducts.ProductName, tOrderAssignments.AssignedToUserUID, mUsers.UserName AS AssignedUserName");
		$this->db->select('tOrders.LastModifiedDateTime,tOrderAssignments.AssignedDateTime');
		$this->db->select('tOrderWorkflows.EntryDatetime,tOrderWorkflows.DueDateTime');
		$this->db->join('tResponseTrack', 'tResponseTrack.OrderUID = tOrders.OrderUID');
		$this->db->where('tResponseTrack.Status', $post['queue_status']);
	...
	function hoiloan_count_all($post)
	{
	...
	function hoiloan_count_filtered($post)
	{
		if (!empty($post['search_value']) || (!empty($post['order']) && isset($post['column_order'][$post['order'][0]['column']]) && !empty($post['column_order'][$post['order'][0]['column']]))) {
			$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
		}
	...
	function KickBackHOIorders($post,$module,$global='') {
		$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
	...
	function Expiredcount_all($post=[])
	{
		...
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetHOIQueue(false,['filtercompletedorders'=>true]);
...
// Expired Orders Filter Function
	...
	function ExpiredHOIOrders($post,$module,$global='')
	{
		$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
	...
	function HOIReworkConditions()
	{
		// JOin tOrderRework
		$this->db->join('tOrderReWork','tOrderReWork.OrderUID = tOrders.OrderUID AND tOrderReWork.WorkflowModuleUID = tOrderWorkflows.WorkflowModuleUID AND tOrderReWork.IsReWorkEnabled = 1');
	}
	...
	*/
	function HOIReworkOrders($post,$module,$global='') {
		$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
		...
		$this->db->select('tOrders.LastModifiedDateTime');
		$this->db->select('tOrderWorkflows.EntryDatetime,tOrderWorkflows.DueDateTime');
		$this->db->select('mUsers.UserName AS EnabledBy, tOrderReWork.EnabledDateTime, tOrderReWork.EnabledRemarks');

		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetHOIQueue(false,['filtercompletedorders'=>true]);

		// HOI Rework Condition
		$this->HOIReworkConditions();

		//Order Queue Permission
		$this->Common_Model->OrdersPermission($this->WorkflowModuleUID,'COMPLETEDCOUNT');
	...
	function HOIReworkOrders_CountAll($post=[])
	{
	...
	function HOIReworkOrders_CountFiltered($post,$module)
	{
		if (!empty($post['search_value']) || (!empty($post['order']) && isset($post['column_order'][$post['order'][0]['column']]) && !empty($post['column_order'][$post['order'][0]['column']]))) {
			$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
		}
