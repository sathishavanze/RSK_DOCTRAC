...
class HOI_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('HOI_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['HOI'];
	}
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->HOI_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['HOI']);
		$data['IsKickBack'] = $this->Common_Model->is_kickback_enabledforworkflow($this->config->item('Workflows')['HOI']);
		$data['IsExpiryOrders'] = $this->Common_Model->is_IsExpiryOrdersRequire_enabledforworkflow($this->config->item('Workflows')['HOI']);
    ...
    //column search
        ...
        */
        ...
        //column order
		$list = $this->HOI_Orders_Model->HOIorders($post,'');
		...
		/**
			...
			*/
		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] 		= "HOI/index/";
		$Mischallenous['AssignButtonClass'] = "HOIPickNewOrder";
...
*/
			...
			{
				$Action = '<a href="HOI/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm HOIPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$myorderslist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->HOI_Orders_Model->count_all($post),
			"recordsFiltered" =>  $this->HOI_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
		...
		$list = $this->HOI_Orders_Model->WorkInProgressOrders($post,'');
...
*/
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs HOIPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->HOI_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->HOI_Orders_Model->inprogress_count_filtered($post),
    ...
    //column search
		$post['column_search'] = array('tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
        //column order
		$list = $this->HOI_Orders_Model->MyOrders($post,'');
...
*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->HOI_Orders_Model->myorders_count_all($post),
			"recordsFiltered" =>  $this->HOI_Orders_Model->myorders_count_filtered($post),
    ...
    //get_post_input_data
		...
		//column search
		$post['column_search'] = array('tOrders.OrderNumber','mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName','mStatus.StatusName','tOrders.PropertyStateCode','b.UserName','tOrderParking.Remarks','tOrderParking.Remainder','tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
		$list = $this->HOI_Orders_Model->parkingorders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->HOI_Orders_Model->parkingorders_count_all($post),
			"recordsFiltered" =>  $this->HOI_Orders_Model->parkingorders_count_filtered($post),
			"data" => $data['myorderslist'],
		);
   ...
   /*
    ...
    //column search
        ...
        */
		...
		$list = $this->HOI_Orders_Model->ExpiredHOIOrders($post,'');
        ...
        */
		...
		if (!empty($DynamicColumns)) 
		{
			$hoilist 				= 	$DynamicColumns['orderslist'];
			$post['column_order']		=	$DynamicColumns['column_order'];
			$post['column_search']		=	$DynamicColumns['column_search'];
			$list = [];
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->HOI_Orders_Model->Expiredcount_all($post),
			"recordsFiltered" =>  $this->HOI_Orders_Model->Expiredcount_filtered($post,''),
	...
	function GetNextOrder()
	{
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST') 
		{
			...
			if ($this->form_validation->run() == true)
			{
				...
				$existingarray = [];
				$ExistingOrders = $this->HOI_Orders_Model->CheckExistingOrders($this->loggedid,$ProjectUID,$Workflow);
				...
				else
				{
					...
					$data = $this->HOI_Orders_Model->AssignOrders($this->loggedid,$ProjectUID,$Workflow);
	...
	function PickExistingOrderCheck()
	{
		...
		$ProjectUID = $this->input->post('ProjectUID');
		$result = $this->HOI_Orders_Model->PickExistingOrderCheck($OrderUID);
		if($result['Status'] == 1)
		{
		...
		{
			$value = $this->HOI_Orders_Model->OrderAssign($OrderUID,$ProjectUID);
			if($value)
			{
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->HOI_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->HOI_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->HOI_Orders_Model->parkingorders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
	...
	function HOIloan_list(){
		...
		$post['queue_status'] = $this->input->post('queue_status');
		$post['length'] = $this->input->post('length');
		$post['start'] = $this->input->post('start');
		$search = $this->input->post('search');
		$post['search_value'] = trim($search['value']);
    ...
    //column search
        ...
        */
        ...
        //column order
		$list = $this->HOI_Orders_Model->Hoi_Loan_Process($post);
...
*/		        $row[] = $myorders->LoanNumber;
		...
		$post = $data['post'];
		$recordsTotal = $this->HOI_Orders_Model->hoiloan_count_all($post);
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $recordsTotal,
			"recordsFiltered" =>  $recordsTotal,
			"data" => $data['hoiwaiting'],
		);

		if (isset($post['search']['value']) &&  $post['search']['value'] !='') {
			$output['recordsFiltered'] = $this->HOI_Orders_Model->hoiloan_count_filtered($post);
		}
    ...
    //column search
        ...
        */
        ...
        //column order
		$list = $this->HOI_Orders_Model->KickBackHOIorders($post,'');
...
*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->HOI_Orders_Model->KickBackcount_all($post),
			"recordsFiltered" =>  $this->HOI_Orders_Model->KickBackcount_filtered($post,''),
			"data" => $data['myorderslist'],
		);
	...
	*/
	function HOIReworkOrdersAjaxList()
	{
        ...
        */
        ...
        //column order
		$list = $this->HOI_Orders_Model->HOIReworkOrders($post,'');
		$no = $post['start'];
		$myorderslist = [];
...
*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->HOI_Orders_Model->HOIReworkOrders_CountAll($post),
			"recordsFiltered" =>  $this->HOI_Orders_Model->HOIReworkOrders_CountFiltered($post,''),
			"data" => $data['myorderslist'],
		);
