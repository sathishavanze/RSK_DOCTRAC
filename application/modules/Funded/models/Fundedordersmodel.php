...
class Fundedordersmodel extends MY_Model {
  ...
  function total_count()
  {
    $this->db->select("1");
      ...
      /*^^^^^ Get Funded Orders Query ^^^^^*/
      $this->Common_Model->GetFundedQueueOrders();
...
$this->Common_Model->FilterByProjectUser($this->RoleUID,$this->loggedid);     
  ...
  function FundedOrders($post,$global='') {
      $this->Common_Model->DynamicColumnsCommonQuery($WorkflowModuleUID = '',TRUE);
    ...
    function GetFundedOrdersExcelRecords($post)
    {
      $this->Common_Model->DynamicColumnsCommonQuery($WorkflowModuleUID = '',TRUE);
