...
<!--     <div class="row">
  ...
  <div class="card-body">
    <ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#Funded" role="tablist">
          Funded Orders
          <span class="badge badge-pill badge-primary" id="Funded-count" style="background-color: #fff;color: #000;"><?php echo $this->Fundedordersmodel->count_all(); ?></span>
    ...
    <div  class="material-datatables" >
      ...
      <table class="table table-hover table-striped" id="Funded">
        <thead>
         <tr>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    Fundedinitialize('false')
    function Fundedinitialize(formdata)
    {
      Funded = $('#Funded').DataTable( {
        scrollX:        true,
        scrollCollapse: true,
        fixedHeader: false,
        scrollY: '100vh',
        ...
        "autoWidth": true,
          ...
          "ajax": {
            "url": "<?php echo base_url('Funded/Fundedorders_ajax_list')?>",
            "type": "POST",
...
$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
  ...
  $.ajax({
   type: "POST",
   url: '<?php echo base_url();?>Funded/WriteExcel',
   xhrFields: {
    responseType: 'blob',
  },
  data: {'formData':formData},
  ...
  {
    var filename = "FundedOrders.csv";
    if (typeof window.chrome !== 'undefined') {
            //Chrome version
            var link = document.createElement('a');
            link.href = window.URL.createObjectURL(data);
            link.download = "FundedOrders.csv";
            link.click();
...
$(document).off('click', '.ExportDocument').on('click', '.ExportDocument', function (e) {  
  ...
  SwalConfirmExport(OrderUID, OrderNumber, currentrow, Funded, LoanNumber);
})
