...
<div class="card mt-40" id="Orderentrycard">
	...
	<div class="card-body">
				...
				<form action="#"  name="UpdateResource_form" id="UpdateResource_form">
					
			<div class="row">
			    <div class="col-md-3">
			       <div class="form-group bmd-form-group">
			        <label for="controller" class="bmd-label-floating">Controller <span class="mandatory"></span></label>
			        <input type="text" class="form-control" id="controller" name="controller" value="<?php echo $UpdateResources->controller ?>" />
			      </div>
			    </div>
			    ...
			    <div class="col-md-3">
			       <div class="form-group bmd-form-group">
			        ...
			        <input type="text" class="form-control" id="FieldName" name="FieldName" value="<?php echo $UpdateResources->FieldName ?>"/>
			    ...
			    <div class="col-md-3">
			       <div class="form-group bmd-form-group">
			        ...
			        <input type="text" class="form-control" id="FieldSection" name="FieldSection" value="<?php echo $UpdateResources->FieldSection ?>" />
			      </div>
			    </div>
				...
				<div class="col-md-3">
		           <div class="form-group bmd-form-group">
		            ...
		            <input type="text" class="form-control" id="IconClass" name="IconClass" value="<?php echo $UpdateResources->IconClass ?>"/>
		          </div>
		        </div>
		    </div>
		    <div class="row mt-10">
		         <div class="col-md-3">
		           <div class="form-group bmd-form-group">
		            ...
		            <input type="number" class="form-control" id="Position" name="Position" value="<?php echo $UpdateResources->Position ?>"/>
		          </div>
		        </div>
               ...
               <div class="form-group bmd-form-group">
                ...
                <select class="select2picker form-control" id="MenuBarType" name="MenuBarType" value="<?php echo $UpdateResources->MenuBarType ?> "> 
                	 <option value=""></option>
                	<?php  
                		$sildbar='';
						if($UpdateResources->MenuBarType == 'sidebar'){
								$sildbar='selected';
							}
                	?>
                  	<option value="sidebar"<?php echo $sildbar ?>>Sidebar</option>

                    <?php  
	                	$jumbobar='';
						if($UpdateResources->MenuBarType == 'jumbobar'){
								$jumbobar='selected';
							}
                	?>
                  	<option value="jumbobar" <?php echo $jumbobar ?>>Jumbobar</option>
                    
                    <?php  
	                	$common='';
						if($UpdateResources->MenuBarType == 'common'){
								$common='selected';
							}
                	?>
                  	<option value="common" <?php echo$common ?>>Common</option>
                  	 <?php  
	                	$submenu1='';
						if($UpdateResources->MenuBarType == 'submenu1'){
								$submenu1='selected';
							}
                	?>


                  	<option value="submenu1" <?php echo$submenu1 ?>>SubMenu1</option>

                  	 <?php  
	                	$submenu2='';
						if($UpdateResources->MenuBarType == 'submenu2'){
								$submenu1='selected';
							}
                	?>

                  	<option value="submenu2" <?php echo$submenu2 ?>>SubMenu2</option>

                </select>
              </div>
            </div>

            <div class="col-md-3 workflow_div" style="<?php if(strtoupper($UpdateResources->FieldSection) != 'ORDERWORKFLOW'){echo 'display: none'; }?>">
               <div class="form-group bmd-form-group">
                ...
                <select class="select2picker form-control" id="WorkflowModuleUID" name="WorkflowModuleUID"> 
                  ...
                  { 
                  	if($UpdateResources->WorkflowModuleUID == $value->WorkflowModuleUID)
                  		{?>
 		 ...
 		 <div class="col-md-3">
           <div class="form-group bmd-form-group">
            ...
            <input type="text" class="form-control" id="NotificationEle" name="NotificationEle" value='<?php echo $UpdateResources->NotificationElement?>'/>
          </div>
        </div>
			    ...
			    <div class="col-md-3">
                    ...
                    <input type="checkbox" id="Active" name="Active" class="Active" <?php if($UpdateResources->Active == 1){ echo "checked"; } ?>>
                    <span class="toggle"></span>
                  	</label>
            		</div>
        		</div>
        		<input type="hidden" name="ResourceUID" value="<?php echo $UpdateResources->ResourceUID ?>" />
		        ...
		        <div class="col-md-3">
		        	...
		        	<select class="select2picker CustomerUID" id="CustomerUID" autocomplete="off" name="CustomerUID[]" multiple="multiple" style="width: 100% !important;">
        				<?php 		        				
        				$CustomerUID_arr = explode(",", $UpdateResources->CustomerUID);
        				foreach ($CustomerDetails as $key => $value) { 
          					if (in_array($value->CustomerUID, $CustomerUID_arr)) {   ?>
				...
				<!-- 	<div class="col-sm-12 form-group pull-right">
						<p class="text-right">
							...
							<button type="button" class="btn btn-space btn-social btn-color btn-twitter UpdateResource">UPDATE Resource</button>
						</p>
					</div> -->
          ...
          <button type="submit" class="btn btn-fill btn-update btn-wd UpdateResource" name="UpdateResource"><i class="icon-floppy-disk pr-10"></i>Update Resources </button>
          </div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).off('click','.UpdateResource').on('click','.UpdateResource', function(e) {

			$(".loginidunique").hide();
			var formdata = $('#UpdateResource_form').serialize();
			button=$(this);
			button_val=$(this).val();
			button_text=$(this).html();
			$.ajax({
				type: "POST",
				url: "<?php echo base_url('Resources/UpdateResourceSave'); ?>",
				data: formdata,
				dataType:'json',
				beforeSend: function () {
