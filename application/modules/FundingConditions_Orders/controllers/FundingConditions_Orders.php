...
class FundingConditions_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('FundingConditions_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['FundingConditions'];
		$this->WorkflowModuleName = 'FundingConditions';
	} 
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->FundingConditions_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['FundingConditions']);
		$data['IsKickBack'] = $this->Common_Model->is_kickback_enabledforworkflow($this->config->item('Workflows')['FundingConditions']);
    ...
    //column search
        ...
        */
		...
		$list = $this->FundingConditions_Orders_Model->FundingConditionsOrders($post,'');
		

      

		$no = $post['start'];
		$FundingConditionslist = [];
		/**
			*Function Description: Dynamic Columns Queues
			*@author Sathis Kannan <sathish.kannan@avanzegroup.com>
			*@since 15.7.2020
			*/
		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] 		= "FundingConditions/index/";
		$Mischallenous['AssignButtonClass'] = "FundingConditionsPickNewOrder";
		...
		{
			$FundingConditionslist 				= 	$DynamicColumns['orderslist'];
		...
		{ 
			...
			{
				$Action = '<a href="FundingConditions/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm FundingConditionsPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$FundingConditionslist[] = $row;
		}



		$data =  array(
			'orderslist' => $FundingConditionslist,
			'post' => $post
		);
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->FundingConditions_Orders_Model->count_all($post),
			"recordsFiltered" =>  $this->FundingConditions_Orders_Model->count_filtered($post,''),
    ...
    //column search
        ...
        */
        ...
        //column order
		$list = $this->FundingConditions_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm FundingConditionsPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->FundingConditions_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->FundingConditions_Orders_Model->inprogress_count_filtered($post),
  ...
  //column order
    ...
    //column search
        ...
        */
		...
		$list = $this->FundingConditions_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->FundingConditions_Orders_Model->myorders_count_all($post),
			"recordsFiltered" =>  $this->FundingConditions_Orders_Model->myorders_count_filtered($post),
   ...
   //column order
    ...
    //column search
        ...
        */
		...
		$list = $this->FundingConditions_Orders_Model->parkingorders($post,'');
		$no = $post['start'];
		$myorderslist = [];
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				$Action = '<a href="FundingConditions/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs FundingConditionsPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}	
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->FundingConditions_Orders_Model->parkingorders_count_all($post),
			"recordsFiltered" =>  $this->FundingConditions_Orders_Model->parkingorders_count_filtered($post),
 ...
 //column order
    ...
    //column search
        ...
        */
		...
		$list = $this->FundingConditions_Orders_Model->KickBackFundingConditionsOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->FundingConditions_Orders_Model->KickBackcount_all($post),
			"recordsFiltered" =>  $this->FundingConditions_Orders_Model->KickBackcount_filtered($post,''),
			"data" => $data['orderslist'],
		);
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->FundingConditions_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
		...
		{
			$list2 = $this->FundingConditions_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->FundingConditions_Orders_Model->parkingorders($post,'');
