					...
					<form action="#"  name="orderform" id="frmordersummary">
						...
						<?php $this->load->view('Ordersummary/Orderdetails'); ?>
							...
							<tbody class="addChecklist">
								...
								$data1['WorkflowUID'] = $this->config->item('Workflows')['POO'];
								$this->load->view('checklist/otherchecklist',$data1);
								?>
							</tbody>	
						</table>
						<p class="custom_add_icon pull-right addchecklistrow" data-count = '1'  data-moduleUID = '<?php echo $this->config->item('Workflows')['POO']; ?>' style="margin-right:10px;" aria-hidden="true">Add New Checklist</p>
						<br>
						<div>
							<?php 
							$data2['OrderUID'] = $OrderSummary->OrderUID; 
							$data2['WorkflowModuleUID'] = $this->config->item('Workflows')['POO'];
