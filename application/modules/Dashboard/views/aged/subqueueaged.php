...
<table  class="display dataTable table table-striped table-hover table-actions-bar subqueue-aging" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
	<thead>
		<tr>
			<th>Sub-Queue</th>
	...
	<tbody>
		...
		{ 
			...
			?>
			<tr>
				<td><b><?php echo $row["QueueName"]; ?></b></td>
				<?php foreach ($DashboardAgingBucketHeader as $AgingHeaderkey => $AgingHeadervalue) { ?>
					...
					<td><a href="javascript:;" title="View Loans" data-heading="Aging - <?php echo $row["QueueName"]; ?>" class="listorders" data-orderid="<?php echo $row[$AgingHeaderkey]; ?>"><?php echo $row[$AgingHeaderkey.'count']; ?></a></td>
					...
					?>
				<?php } ?>
				<td><span class="text-bold"><?php echo !empty($totalsum) && !empty($noofvalues) ? $totalsum/$noofvalues : 0; ?></span></td>
