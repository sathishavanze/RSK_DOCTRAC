...
class Submissions_Orders_Model extends MY_Model {
	...
	function total_count()
	{
		...
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetSubmissionsOrders(false);

		// Gatekeeping fetch order conditions
		$this->db->group_start();
			$this->db->group_start();
				$this->SubmissionCondition();
			$this->db->group_end();
			$this->db->or_group_start();
				$this->db->where("EXISTS (SELECT 1 FROM tOrderParking WHERE OrderUID = tOrders.OrderUID AND WorkflowModuleUID = '".$this->WorkflowModuleUID."' AND IsCleared = 0)",NULL,FALSE);
			$this->db->group_end();
		$this->db->group_end();
	...
	function count_all($post = FALSE)
	{
		...
		$this->Common_Model->GetSubmissionsOrders();
		...
		$this->db->group_start();
		$this->db->where('tOrders.LoanType','FHA/VA');
		$this->db->or_where('tOrders.LoanType','FHA');
		$this->db->or_where('tOrders.LoanType','');
		$this->db->group_end();
	...
	function SubmissionsOrders($post,$module,$global='')
	{
		$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
	...
	function VASubmissionsOrders($post,$module,$global='')
	{
		$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
		...
		$this->db->where("NOT EXISTS (SELECT 1 FROM tOrderParking WHERE OrderUID = tOrders.OrderUID AND WorkflowModuleUID = '".$this->WorkflowModuleUID."' AND IsCleared = 0)",NULL,FALSE);
		$this->db->where('tOrders.LoanType','VA');
	...
	// New FHA Orders
	function VAcount_all($post = FALSE)
	{
		$this->db->select("1");
	...
	function VAcount_filtered($post,$module)
	{
		$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
		$this->db->select("1");
