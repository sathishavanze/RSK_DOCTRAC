...
class Submissions_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('Submissions_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['Submissions'];
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->Submissions_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['Submissions']);
	...
	function neworders_ajax_list(){
		...
		$list = $this->Submissions_Orders_Model->VASubmissionsOrders($post,'');

		$no = $post['start'];
		$Submissionslist = [];

		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] 		= "Submissions/index/";
		$Mischallenous['AssignButtonClass'] = "SubmissionsPickNewOrder";
		...
		{
			$Submissionslist 				= 	$DynamicColumns['orderslist'];
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="Submissions/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm SubmissionsPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$Submissionslist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Submissions_Orders_Model->VAcount_all($post),
			"recordsFiltered" =>  $this->Submissions_Orders_Model->VAcount_filtered($post,''),
	...
	function FHAneworders_ajax_list(){
		//Advanced Search
		$post['advancedsearch'] = $this->input->post('formData');
		//Advanced Search
		//get_post_input_data
		...
		$list = $this->Submissions_Orders_Model->SubmissionsOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Submissions_Orders_Model->count_all($post),
			"recordsFiltered" =>  $this->Submissions_Orders_Model->count_filtered($post,''),
			"data" => $data['orderslist'],
		);
	...
	function workinginprogress_ajax_list()
	{
        ...
        //column order
		$list = $this->Submissions_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm SubmissionsPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Submissions_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->Submissions_Orders_Model->inprogress_count_filtered($post),
	...
	function myorders_ajax_list()
	{
		...
		$list = $this->Submissions_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Submissions_Orders_Model->myorders_count_all($post),
			"recordsFiltered" =>  $this->Submissions_Orders_Model->myorders_count_filtered($post),
	...
	function parkingorders_ajax_list()
	{
		...
		$list = $this->Submissions_Orders_Model->parkingorders($post,'');
		$no = $post['start'];
		$myorderslist = [];
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				$Action = '<a href="Submissions/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs SubmissionsPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}	
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Submissions_Orders_Model->parkingorders_count_all($post),
			"recordsFiltered" =>  $this->Submissions_Orders_Model->parkingorders_count_filtered($post),
	...
	function WriteExcel()
	{
		...
		if($post['formData']['Status'] == 'VAorderslist')
		{
			$list = $this->Submissions_Orders_Model->SubmissionsOrders($post,'');
		...
		{
			$list1 = $this->Submissions_Orders_Model->WorkInProgressOrders($post,'');
			$data = [];
		...
		{
			$list2 = $this->Submissions_Orders_Model->MyOrders($post,'');
			$data = [];
			$data[] = array('Order No','Loan No','Client','Loan Type','Milestone','Current Status','State','Aging','Due Date Time','Last Modified Date Time');
		...
		{
			$list3 = $this->Submissions_Orders_Model->parkingorders($post,'');
			$data = [];
	...
	*/
	function widgetGetPendingOrdersCount() {
		...
		if ($filterlist == '#orderslist') {
			$FilterCounts['ModelName'] = 'Submissions_Orders_Model';
			$FilterCounts['FunctionName'] = 'VAcount_all';
		} elseif ($filterlist == '#FHAorderslist') {
			$FilterCounts['ModelName'] = 'Submissions_Orders_Model';
