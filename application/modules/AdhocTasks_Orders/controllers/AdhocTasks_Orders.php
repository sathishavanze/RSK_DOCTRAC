...
class AdhocTasks_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('AdhocTasks_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['AdhocTasks'];
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->AdhocTasks_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['AdhocTasks']);
	...
	function neworders_ajax_list(){
		...
		$list = $this->AdhocTasks_Orders_Model->AdhocTasksOrders($post,'');

		$no = $post['start'];
		$AdhocTaskslist = [];

		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] 		= "AdhocTasks/index/";
		$Mischallenous['AssignButtonClass'] = "AdhocTasksPickNewOrder";
		...
		{
			$AdhocTaskslist 				= 	$DynamicColumns['orderslist'];
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="AdhocTasks/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm AdhocTasksPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$AdhocTaskslist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->AdhocTasks_Orders_Model->count_all(),
			"recordsFiltered" =>  $this->AdhocTasks_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
        ...
        //column order
		$list = $this->AdhocTasks_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm AdhocTasksPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->AdhocTasks_Orders_Model->inprogress_count_all(),
			"recordsFiltered" =>  $this->AdhocTasks_Orders_Model->inprogress_count_filtered($post),
	...
	function myorders_ajax_list()
	{
		...
		$list = $this->AdhocTasks_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->AdhocTasks_Orders_Model->myorders_count_all(),
			"recordsFiltered" =>  $this->AdhocTasks_Orders_Model->myorders_count_filtered($post),
	...
	function parkingorders_ajax_list()
	{
		...
		$list = $this->AdhocTasks_Orders_Model->parkingorders($post,'');
		$no = $post['start'];
		$myorderslist = [];
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				$Action = '<a href="AdhocTasks/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs AdhocTasksPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}	
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->AdhocTasks_Orders_Model->parkingorders_count_all(),
			"recordsFiltered" =>  $this->AdhocTasks_Orders_Model->parkingorders_count_filtered($post),
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->AdhocTasks_Orders_Model->WorkInProgressOrders($post,'');
			$data = [];
		...
		{
			$list2 = $this->AdhocTasks_Orders_Model->MyOrders($post,'');
			$data = [];
			$data[] = array('Order No','Loan No','Client','Loan Type','Milestone','Current Status','State','Aging','Due Date Time','Last Modified Date Time');
		...
		{
			$list3 = $this->AdhocTasks_Orders_Model->parkingorders($post,'');
			$data = [];
