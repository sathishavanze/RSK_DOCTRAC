...
class TitleTeam_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('TitleTeam_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['TitleTeam'];
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->TitleTeam_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['TitleTeam']);
		$data['IsKickBack'] = $this->Common_Model->is_kickback_enabledforworkflow($this->config->item('Workflows')['TitleTeam']);
		$data['IsExpiryOrders'] = $this->Common_Model->is_IsExpiryOrdersRequire_enabledforworkflow($this->config->item('Workflows')['TitleTeam']);
    ...
    //column search
        ...
        */
		...
		$list = $this->TitleTeam_Orders_Model->TitleTeamOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="TitleTeam/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm TitleTeamPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$myorderslist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->TitleTeam_Orders_Model->count_all($post),
			"recordsFiltered" =>  $this->TitleTeam_Orders_Model->count_filtered($post,''),
    ...
    //column order
        ...
        */
		...
		$list = $this->TitleTeam_Orders_Model->WorkInProgressOrders($post,'');
...
*/
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs TitleTeamPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
		    }
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->TitleTeam_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->TitleTeam_Orders_Model->inprogress_count_filtered($post),
    ...
    //column search
        ...
        */
		...
		$list = $this->TitleTeam_Orders_Model->MyOrders($post,'');
...
*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->TitleTeam_Orders_Model->myorders_count_all($post),
			"recordsFiltered" =>  $this->TitleTeam_Orders_Model->myorders_count_filtered($post),
    ...
    //get_post_input_data
        ...
        */
		...
		$list = $this->TitleTeam_Orders_Model->parkingorders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->TitleTeam_Orders_Model->parkingorders_count_all($post),
			"recordsFiltered" =>  $this->TitleTeam_Orders_Model->parkingorders_count_filtered($post),
			"data" => $data['myorderslist'],
		);
...
function Expiredorders_ajax_list(){
    ...
    //column search
        ...
        */
		...
		$list = $this->TitleTeam_Orders_Model->ExpiredOrders($post,'');
        ...
        */
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->TitleTeam_Orders_Model->Expiredcount_all($post),
			"recordsFiltered" =>  $this->TitleTeam_Orders_Model->Expiredcount_filtered($post,''),
	...
	function GetNextOrder()
	{
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST') 
		{
			...
			if ($this->form_validation->run() == true)
			{
				...
				$existingarray = [];
				$ExistingOrders = $this->TitleTeam_Orders_Model->CheckExistingOrders($this->loggedid,$ProjectUID,$Workflow);
				...
				else
				{
					...
					$data = $this->TitleTeam_Orders_Model->AssignOrders($this->loggedid,$ProjectUID,$Workflow);
	...
	function PickExistingOrderCheck()
	{
		...
		$ProjectUID = $this->input->post('ProjectUID');
		$result = $this->TitleTeam_Orders_Model->PickExistingOrderCheck($OrderUID);
		if($result['Status'] == 1)
		{
		...
		{
			$value = $this->TitleTeam_Orders_Model->OrderAssign($OrderUID,$ProjectUID);
			if($value)
			{
    ...
    //column search
        ...
        */
		...
		$list = $this->TitleTeam_Orders_Model->KickBackTitleTeamOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->TitleTeam_Orders_Model->KickBackcount_all($post),
			"recordsFiltered" =>  $this->TitleTeam_Orders_Model->KickBackcount_filtered($post,''),
			"data" => $data['myorderslist'],
		);
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->TitleTeam_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->TitleTeam_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->TitleTeam_Orders_Model->parkingorders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
