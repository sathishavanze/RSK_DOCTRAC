...
<div class="card mt-40">
	<div class="card-header card-header-danger card-header-icon">
		<div class="card-icon">
			<h4 class="card-title">Third Party Orders</h4>
		</div>
	...
	<div class="card-body" id="filter-bar">
		...
		<ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
			<li class="nav-item thirdparrtyQueue">
				<a class="nav-link" data-toggle="tab" href="#orderslist" role="tablist">
					New Orders
					<span class="badge badge-pill badge-primary" id="newdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->ThirdParty_Orders_Model->count_all(); ?></span>
			...
			<?php
				if (in_array($this->RoleType, $this->config->item('Internal Roles'))) {
					...
					<li class="nav-item thirdparrtyQueue">
						<a class="nav-link" data-toggle="tab" href="#workinprogresslist" role="tablist">
							Assigned Orders
							<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->ThirdParty_Orders_Model->inprogress_count_all(); ?></span>
						</a>
					</li>
					<li class="nav-item thirdparrtyQueue">
						<a class="nav-link " data-toggle="tab" href="#myorderslist" role="tablist">
							My Orders
							<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->ThirdParty_Orders_Model->myorders_count_all(); ?></span>
						</a>
					</li>
				...
				<li class="nav-item">
					<a class="nav-link " data-toggle="tab" href="#parkingorderslist" role="tablist">
						Parking Orders
						<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->ThirdParty_Orders_Model->parkingorders_count_all(); ?></span>
					</a>
				</li>
			<?php } ?>
				<!-- Completed Orders -->
		...
		<div class="tab-content tab-space customtabpane">
			...
			$viewdata['NewOrdersTableName'] = "tblThirdParty";
			...
			<script type="text/javascript">
				var tblThirdParty = false;
				$(function() {
					$("select.select2picker").select2({
						//tags: false,
						theme: "bootstrap",
					});
					$('#tblThirdParty').DataTable().destroy();
				});
			...
			$(document).off('click','.PickNewOrder_old').on('click','.PickNewOrder_old',function(){
			    ...
			    $.ajax({
					...
					$(document).off('click','.search').on('click','.search',function()
					{
					      ...
					      var ServiceType = $('#adv_ServiceType option:selected').val();
					...
					var LoanNo = $('#adv_LoanNo').val();
					      ...
					      if((ProjectUID == '') && (MilestoneUID == '') && (StateUID == '') && (LoanNo == '') && (ServiceType == '') && (PackageUID == '') && (InputDocTypeUID == '') && (CustomerUID == '') && (FromDate == '') && (ToDate == '') && (ProductUID ==''))
					      {
					       ...
					       var formData = ({ 'ProductUID':ProductUID,'MilestoneUID':MilestoneUID,'StateUID':StateUID,'LoanNo':LoanNo,'ProjectUID': ProjectUID ,'PackageUID': PackageUID,'InputDocTypeUID': InputDocTypeUID,'ServiceType': ServiceType,'CustomerUID':CustomerUID,'FromDate':FromDate,'ToDate':ToDate}); 
	...
	var filterlist = $("#filter-bar .active").attr("href");
		...
		var LoanNo = $('#adv_LoanNo').val();
					...
					{
						var formData = ({ 'ProductUID':ProductUID, 'MilestoneUID':MilestoneUID, 'StateUID':StateUID, 'LoanNo':LoanNo,'ProjectUID': ProjectUID ,'PackageUID': PackageUID,'InputDocTypeUID': InputDocTypeUID,'ServiceType': ServiceType,'CustomerUID':CustomerUID,'FromDate':FromDate,'ToDate':ToDate,'Status':filter}); 
					}
					...
					// } else {
					// 	action_url = '<?php echo base_url();?>ThirdParty_Orders/WriteExcel';
					// }
					...
					*/
					...
					if (typeof window.chrome !== 'undefined') {
				            ...
				            link.download = "ThirdParty "+ filename;
				            link.click();
				...
				function initialize(formData)
				{
					...
					tblThirdParty = $('#tblThirdParty').DataTable( {
						scrollX:        true,
						scrollCollapse: true,
						fixedHeader: false,
						scrollY: '100vh',
						...
						"ajax": {
							"url": "<?php echo base_url('ThirdParty_Orders/ThirdPartyorders_ajax_list')?>",
							"type": "POST",
							"data" : {'formData':formData} 
						},
						"columnDefs": [ {
				...
				function workinprogressinitialize(formData){
					...
					// Load data for the table's content from an Ajax source
					"ajax": {
						"url": "<?php echo base_url('ThirdParty_Orders/workinginprogress_ajax_list')?>",
						"type": "POST",
						"data" : {'formData':formData} 
					},
					"columnDefs": [ {
				...
				function myordersinitialize(formData){
					...
					// Load data for the table's content from an Ajax source
					"ajax": {
						"url": "<?php echo base_url('ThirdParty_Orders/myorders_ajax_list')?>",
						"type": "POST",
						"data" : {'formData':formData} 
					},
					"columnDefs": [ {
	...
	<?php if (in_array($this->RoleType, $this->config->item('Internal Roles'))) { ?>
		...
		parkingorderstable = $('#parkingorderstable').DataTable( {
			...
			// Load data for the table's content from an Ajax source
			"ajax": {
				"url": "<?php echo base_url('ThirdParty_Orders/parkingorders_ajax_list')?>",
				"type": "POST",
				"data" : {'formData':formData} 
			},
			"columnDefs": [ {
...
<script type="text/javascript">
	$(document).off('click','.ThirdPartyPickNewOrder').on('click','.ThirdPartyPickNewOrder',function(){
	...
	.done(function(response) {
		...
		if(response.validation_error == 2)
		{
			...
			setTimeout(function(){ 
				...
				triggerpage('ThirdPartyTeam/index/'+OrderUID);
