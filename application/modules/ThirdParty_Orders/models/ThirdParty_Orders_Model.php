...
class ThirdParty_Orders_Model extends MY_Model {
	...
	{
		$parkingenabled = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['ThirdPartyTeam']);

		$this->db->select("1");


		/*^^^^^ Get Orders Query ^^^^^*/
		$this->Common_Model->GetThirdPartyQueue();

		//filter order when parking disabled
		if(empty($parkingenabled)) {
			$this->db->where("NOT EXISTS (SELECT 1 FROM tOrderParking WHERE tOrderParking.OrderUID = tOrders.OrderUID AND tOrderParking.WorkflowModuleUID='".$this->config->item('Workflows')['ThirdPartyTeam']."' AND tOrderParking.IsCleared = 0 )",NULL,FALSE);
		}
  ...
  // New Orders
	function count_all()
	{
		...
		/*^^^^^ Get Orders Query ^^^^^*/
		$this->Common_Model->GetThirdPartyQueue();
		$this->db->where("NOT EXISTS (SELECT 1 FROM tOrderParking WHERE OrderUID = tOrders.OrderUID AND WorkflowModuleUID = '".$this->config->item('Workflows')['ThirdPartyTeam']."' AND IsCleared = 0)",NULL,FALSE);
	...
	function ThirdPartyOrders($post,$module,$global='') {
		...
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetThirdPartyQueue();
		$this->db->where("NOT EXISTS (SELECT OrderUID FROM tOrderParking WHERE OrderUID = tOrders.OrderUID AND WorkflowModuleUID = '".$this->config->item('Workflows')['ThirdPartyTeam']."' AND IsCleared = 0)",NULL,FALSE);
      ...
      // Datatable OrderBy
		...
		$MetricsOrderBy = $this->Common_Model->getMetricsDependentworkflows($this->parameters['DefaultClientUID'], $this->config->item('Workflows')['ThirdPartyTeam']);
  ...
  // Work in Progress
     ...
     //parking orders
		...
		function parkingorders_count_all()
		{
			...
			/*^^^^^ Get MyOrders Query ^^^^^*/
			$this->Common_Model->GetThirdPartyQueue(false);
			$this->db->join("tOrderParking","tOrderParking.OrderUID=tOrders.OrderUID and tOrderParking.WorkflowModuleUID='".$this->config->item('Workflows')['ThirdPartyTeam']."' AND tOrderParking.IsCleared = 0 ");
