		...
		<ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
			<li class="nav-item">
				<a class="nav-link active" data-toggle="tab" href="#review" role="tablist">
					Withdrawal Orders
					<span class="badge badge-pill badge-primary" id="cancelled-count" style="background-color: #fff;color: #000;"><?php echo $this->Withdrawal_Orders_model->count_all(); ?></span>
...
<script type="text/javascript">
		...
		function cancelledinitialize(formdata)
		{
			review = $('#review').DataTable( {
				...
				"autoWidth": true,
					...
					"ajax": {
						"url": "<?php echo base_url('Withdrawal_Orders/Withdrawal_Orders_ajax_list')?>",
						"type": "POST",
	...
	$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
		...
		$.ajax({
			type: "POST",
			url: '<?php echo base_url();?>Withdrawal_Orders/WriteExcel',
			xhrFields: {
				responseType: 'blob',
			},
			data: {'formData':formData},
			...
			success: function(data)
			{
				...
				if (typeof window.chrome !== 'undefined') {
						...
						link.download = "Withdrawal_Orders.csv";
						link.click();
