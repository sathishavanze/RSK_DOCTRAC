...
class PE_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('PE_Orders_Model');
	}
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->PE_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['PE']);
		$this->load->view($this->input->is_ajax_request() ? $data['content'] : 'page', $data);
	}

	function PEorders_ajax_list(){
		//Advanced Search
    ...
    //column order
		...
		$list = $this->PE_Orders_Model->PEOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="PE/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i></a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm PEPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i></button>';
			}
			$row[] = $Action;
			$PElist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->PE_Orders_Model->count_all(),
			"recordsFiltered" =>  $this->PE_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
  	...
  	//column order
        ...
        //column order
		$list = $this->PE_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm PEPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i></button>';
			}
			$row[] = $Action;
			}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->PE_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->PE_Orders_Model->inprogress_count_filtered($post),
    ...
    //column order and search
		$post['column_order'] = array('tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
		$post['column_search'] = array('tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
		$list = $this->PE_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->PE_Orders_Model->myorders_count_all(),
			"recordsFiltered" =>  $this->PE_Orders_Model->myorders_count_filtered($post),
...
/*********************************
 * /**
			*Function Description: Parking Order Ajax list
...
//get_post_input_data
	...
	//column search
	$post['column_search'] = array('tOrders.OrderNumber','mCustomer.CustomerName','tOrders.LoanNumber','tOrders.LoanType','mMilestone.MilestoneName','mStatus.StatusName','tOrders.PropertyStateCode','b.UserName','tOrderParking.Remarks','tOrderParking.Remainder','tOrderWorkflows.EntryDatetime','tOrderWorkflows.DueDateTime','tOrders.LastModifiedDateTime');
	$list = $this->PE_Orders_Model->parkingorders($post,'');
	...
	foreach ($list as $myorders)
	{
		...
		if($this->loggedid == $myorders->AssignedToUserUID)
		{
			...
			$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs PEPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i></button>';
		}
		$row[] = $Action;
		$myorderslist[] = $row;
	}
	...
	$output = array(
		"draw" => $post['draw'],
		"recordsTotal" => $this->PE_Orders_Model->parkingorders_count_all(),
		"recordsFiltered" =>  $this->PE_Orders_Model->parkingorders_count_filtered($post),
...
/**********************************Parking Order Ajax List End****************************************** */
	function WriteExcel()
	{
		...
		{
			$list1 = $this->PE_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->PE_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->PE_Orders_Model->parkingorders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
