...
class UnderWriter_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('UnderWriter_Orders_Model');
	}
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->UnderWriter_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['UnderWriter']);
		$this->load->view($this->input->is_ajax_request() ? $data['content'] : 'page', $data);
	}

	function UnderWriterorders_ajax_list()
	{
		//Advanced Search
    	...
    	//get_post_input_data
    	//column order
    	$post['column_order'] = array('tOrders.OrderDueDate','tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','mMilestone.MilestoneName','tOrders.LoanType', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrders.LastModifiedDateTime');
    	$post['column_search'] = array('tOrders.OrderDueDate','tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','mMilestone.MilestoneName','tOrders.LoanType', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrders.LastModifiedDateTime');
        //column order
        $list = $this->UnderWriter_Orders_Model->UnderWriterOrders($post,'');
...
*/
		        ...
		        {
		        	$Action = '<a href="UnderWriter/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i></a>';
		        }else{

		       		$Action = '<button class="btn btn-info btn-sm UnderWriterPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i></button>';
		        }
		        $row[] = $Action;
		        $myorderslist[] = $row;
        }
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->UnderWriter_Orders_Model->count_all(),
			"recordsFiltered" =>  $this->UnderWriter_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
    	...
    	//get_post_input_data
    	//column order
    	$post['column_order'] = array('tOrders.OrderDueDate','tOrders.OrderNumber','mCustomer.CustomerName','tOrders.LoanNumber','mMilestone.MilestoneName','tOrders.LoanType','mStatus.StatusName','tOrders.PropertyStateCode','mUsers.UserName','tOrderAssignments.AssignedDateTime','tOrders.LastModifiedDateTime');
    	$post['column_search'] = array('tOrders.OrderDueDate','tOrders.OrderNumber','mCustomer.CustomerName','tOrders.LoanNumber','mMilestone.MilestoneName','tOrders.LoanType','mStatus.StatusName','tOrders.PropertyStateCode','mUsers.UserName','tOrderAssignments.AssignedDateTime','tOrders.LastModifiedDateTime');
        //column order
        $list = $this->UnderWriter_Orders_Model->WorkInProgressOrders($post,'');
...
*/
		        if($this->loggedid == $myorders->AssignedToUserUID)
		        {
		        	$Action = '<a href="UnderWriter/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i></a>';
		       		...
		       		$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs UnderWriterPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i></button>';
		        }
		        $row[] = $Action;
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->UnderWriter_Orders_Model->inprogress_count_all(),
			"recordsFiltered" =>  $this->UnderWriter_Orders_Model->inprogress_count_filtered($post),
	...
	function myorders_ajax_list()
	{
    	...
    	$post['column_search'] = array('tOrders.OrderDueDate','tOrders.OrderNumber', 'mCustomer.CustomerName','tOrders.LoanNumber','mMilestone.MilestoneName','tOrders.LoanType', 'mStatus.StatusName', 'tOrders.PropertyStateCode', 'tOrders.LastModifiedDateTime');
        //column order
        $list = $this->UnderWriter_Orders_Model->MyOrders($post,'');
...
*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->UnderWriter_Orders_Model->myorders_count_all(),
			"recordsFiltered" =>  $this->UnderWriter_Orders_Model->myorders_count_filtered($post),
    ...
    //get_post_input_data
		...
		$list = $this->UnderWriter_Orders_Model->parkingorders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->UnderWriter_Orders_Model->parkingorders_count_all(),
			"recordsFiltered" =>  $this->UnderWriter_Orders_Model->parkingorders_count_filtered($post),
			"data" => $data['myorderslist'],
		);
	...
	function GetNextOrder()
	{
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST') 
		{
			...
			if ($this->form_validation->run() == true)
			{
					...
					$existingarray = [];
					$ExistingOrders = $this->UnderWriter_Orders_Model->CheckExistingOrders($this->loggedid,$ProjectUID,$Workflow);
					...
					else
					{
						...
						$data = $this->UnderWriter_Orders_Model->AssignOrders($this->loggedid,$ProjectUID,$Workflow);
	...
	function PickExistingOrderCheck()
	{
		...
		$ProjectUID = $this->input->post('ProjectUID');
		$result = $this->UnderWriter_Orders_Model->PickExistingOrderCheck($OrderUID);
		if($result['Status'] == 1)
		{
		...
		{
			$value = $this->UnderWriter_Orders_Model->OrderAssign($OrderUID,$ProjectUID);
			if($value)
			{
	...
	function WriteExcel()
	{
		...
		{
		 $list1 = $this->UnderWriter_Orders_Model->WorkInProgressOrders($post,'');	
		 /**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->UnderWriter_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->UnderWriter_Orders_Model->parkingorders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
