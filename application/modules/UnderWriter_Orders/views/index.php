...
<div class="card mt-40">
	<div class="card-header card-header-danger card-header-icon">
		<div class="card-icon">
			<h4 class="card-title">UnderWriter Orders</h4>
		</div>
	...
	<div class="card-body" id="filter-bar">
		...
		<ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
			<li class="nav-item ">
				<a class="nav-link" data-toggle="tab" href="#orderslist" role="tablist">
					New Orders
					<span class="badge badge-pill badge-primary" id="newdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->UnderWriter_Orders_Model->count_all(); ?></span>
			...
			<?php
				if (in_array($this->RoleType, $this->config->item('Internal Roles'))) {
					...
					<li class="nav-item">
						<a class="nav-link" data-toggle="tab" href="#workinprogresslist" role="tablist">
							Assigned Orders
							<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->UnderWriter_Orders_Model->inprogress_count_all(); ?></span>
						</a>
					</li>
					<li class="nav-item">
						<a class="nav-link " data-toggle="tab" href="#myorderslist" role="tablist">
							My Orders
							<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->UnderWriter_Orders_Model->myorders_count_all(); ?></span>
						</a>
					</li>
				...
				<li class="nav-item">
					<a class="nav-link " data-toggle="tab" href="#parkingorderslist" role="tablist">
						Parking Orders
						<span class="badge badge-pill badge-primary" id="workinprogressdoc-checkin-count" style="background-color: #fff;color: #000;"><?php echo $this->UnderWriter_Orders_Model->parkingorders_count_all(); ?></span>
					</a>
				</li>
			<?php } ?>
				<!-- Completed Orders -->
		...
		<div class="tab-content tab-space customtabpane">
			...
			$viewdata['NewOrdersTableName'] = "tblUnderWriter";
			...
			<script type="text/javascript">
				var tblUnderWriter = false;
				$(function() {
					$("select.select2picker").select2({
						//tags: false,
						theme: "bootstrap",
					});
					$('#tblUnderWriter').DataTable().destroy();
				});
...
var MilestoneUID = $('#adv_MilestoneUID option:selected').val();
		...
		var LoanNo = $('#adv_LoanNo').val();
					...
					// } else {
					// 	action_url = '<?php echo base_url();?>UnderWriter_Orders/WriteExcel';
					// }
					...
					$.ajax({
						...
						*/
						...
						if (typeof window.chrome !== 'undefined') {
				            ...
				            link.download = "UnderWriter "+ filename;
				            link.click();
				...
				function initialize(formData)
				{
					...
					tblUnderWriter = $('#tblUnderWriter').DataTable( {
						scrollX:        true,
						scrollCollapse: true,
						scrollY: '100vh',
						fixedHeader: false,
						...
						"ajax": {
							"url": "<?php echo base_url('UnderWriter_Orders/UnderWriterorders_ajax_list')?>",
							"type": "POST",
							"data" : {'formData':formData} 
						},
						"columnDefs": [ {
				...
				function workinprogressinitialize(formData){
					...
					// Load data for the table's content from an Ajax source
					"ajax": {
						"url": "<?php echo base_url('UnderWriter_Orders/workinginprogress_ajax_list')?>",
						"type": "POST",
						"data" : {'formData':formData} 
					},
					"columnDefs": [ {
				...
				function myordersinitialize(formData){
					...
					// Load data for the table's content from an Ajax source
					"ajax": {
						"url": "<?php echo base_url('UnderWriter_Orders/myorders_ajax_list')?>",
						"type": "POST",
						"data" : {'formData':formData} 
					},
					"columnDefs": [ {
	...
	<?php if (in_array($this->RoleType, $this->config->item('Internal Roles'))) { ?>
		...
		parkingorderstable = $('#parkingorderstable').DataTable( {
			...
			// Load data for the table's content from an Ajax source
			"ajax": {
				"url": "<?php echo base_url('UnderWriter_Orders/parkingorders_ajax_list')?>",
				"type": "POST",
				"data" : {'formData':formData} 
			},
			"columnDefs": [ {
	...
	$(document).off('click','.UnderWriterPickNewOrder').on('click','.UnderWriterPickNewOrder',function(){
	...
	.done(function(response) {
		...
		if(response.validation_error == 2)
		{
			...
			setTimeout(function(){ 
				...
				triggerpage('UnderWriter/index/'+OrderUID);
