...
<div class="modal-dialog modal-md">
  ...
  <script type="text/javascript">
				...
				$(document).off('click','#Doc-Order-Attach, #Doc-Order-Attach-Merge').on('click','#Doc-Order-Attach, #Doc-Order-Attach-Merge',function(e){
					...
					$.ajax({
						url: 'DocumentTracking/AttachDocumentToOrder',
						type: 'POST',
						dataType: 'json',
				...
				$(document).off('click','#btndeletedocs').on('click','#btndeletedocs',function(e){
					...
					$.ajax({
						url: 'DocumentTracking/DeleteDocuments',
						type: 'POST',
						dataType: 'json',
				...
				$(document).off('click', '#btnsendemail-modal').on('click', '#btnsendemail-modal', function (e) {
					...
					if (DocumentTrackingUID.length == 1) {
						...
						SendAsyncAjaxRequest('POST', 'DocumentTracking/GetCustomerLenderEmail', requestdata, 'json', true, true, function () {}).
				...
				$(document).off('submit', '#frmsendmail').on('submit', '#frmsendmail', function (e) {
					...
					$.ajax({
						url: 'DocumentTracking/SendEmail',
						type: 'POST',
						dataType: 'json',
						data: formdata,
						contentType: false,
				...
				$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
					...
					$.ajax({
						type: "POST",
						url: '<?php echo base_url();?>DocumentTracking/DocumentExcel',
						xhrFields: {
							responseType: 'blob',
						},
				...
				function initialize(formData)
				{
					...
					tbl_DocumentTracking = $('#tbl_DocumentTracking').DataTable( {
						...
						"ajax": {
							"url": "<?php echo base_url('DocumentTracking/TrackingDocuments_AjaxList')?>",
							"type": "POST",
							"data" : {'formData':formData} 
						},
						"columnDefs": [ {
				...
				function Ordersinitialize(formData){
					...
					// Load data for the table's content from an Ajax source
					"ajax": {
						"url": "<?php echo base_url('DocumentTracking/Orders_ajax_list')?>",
						"type": "POST",
						"data" : {'formData':formData} 
					},
					"drawCallback": function( settings ) {
