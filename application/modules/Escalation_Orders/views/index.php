    ...
    <ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#review" role="tablist">
          Escalation Orders
          <span class="badge badge-pill badge-primary" id="cancelled-count" style="background-color: #fff;color: #000;"><?php echo $this->Escalation_Orders_model->count_all(); ?></span>
...
cancelledinitialize('false')
    function cancelledinitialize(formdata)
    {
          ...
          "ajax": {
            "url": "<?php echo base_url('Escalation_Orders/GetEsclationQueue_ajax_list')?>",
            "type": "POST",
...
$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
  ...
  $.ajax({
    ...
    url: '<?php echo base_url();?>Escalation_Orders/WriteExcel',
          xhrFields: {
      responseType: 'blob',
    },
     data: {'formData':formData},
    ...
    success: function(data)
    {
        // var filename = "CancelledOrders.csv";
         var filename = "EscalationOrders.csv";
        if (typeof window.chrome !== 'undefined') {
            //Chrome version
            var link = document.createElement('a');
            link.href = window.URL.createObjectURL(data);
            link.download = "EscalationOrders.csv";
            link.click();
