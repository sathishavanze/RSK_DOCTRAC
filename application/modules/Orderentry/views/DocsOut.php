...
<div class="card mt-40" id="Orderentrycard">
	...
	<div class="card-body">
		<ul class="nav nav-pills nav-pills-danger customtab entrytab" role="tablist">
			...
			<li class="nav-item">
				<a class="nav-link active ajaxload" href="<?php echo base_url(); ?>Orderentry/DocsOut" role="tablist">
					Docs Out 
				</a>
			</li>
		</ul>
		...
		<div class="tab-content tab-space customtabpane">
			...
			<div class="tab-pane active" id="bulkentry">
				...
				<div class="row">
					...
					<div class="col-md-3">
						<div class="form-group bmd-form-group">
							<label for="bulk_importtype" class="bmd-label-floating">Import Type<span class="mandatory"></span></label>
							<select class="select2picker form-control"  id="bulk_importtype" name="bulk_importtype">
...
<script type="text/javascript">
    ...
    $(document).off('change', '#bulk_products').on('change', '#bulk_products', function (e) {
    	...
    	var Entrytype = $(this).find(':selected').attr('data-docsoutbulkupdateformat');
    	var Entrytypefile = $(this).find(':selected').attr('data-docsoutbulkupdatetemplatename');
    	var Entrytypexmlfile = $(this).find(':selected').attr('data-typexmlname');

    	if (Entrytype == 'LOP-Standard') {
    		$('#file-standard-import').show();
    		$('#standardpreviewfile').show();
    		$('#standardpreviewfile').prop('disabled', false);
    	}
      ...
      error: function(jqXHR, textStatus, errorThrown) {
		...
		$(document).off('click',  '#preview').on('click',  '#preview', function (event) {
			...
			if(bulk_importtype == '2'){
				...
				url = 'Orderentry/Preview_DocsOutUpdate';

			}else{

				// url="Orderentry/preview_bulkentry";
		...
		$(document).off('click',  '#bulk_save').on('click',  '#bulk_save', function (event) {
			...
			if(bulk_importtype == '2'){
				...
				url = 'Orderentry/docsoutupdate_bulk';
...
/*@author Parthasarathy @purpose bulk_importtype*/
/*change event for #bulk_importtype*/
$(document).off('change', '#bulk_importtype').on('change', '#bulk_importtype', function(e){
	e.preventDefault();
