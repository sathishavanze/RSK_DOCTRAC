...
class WorkflowQueueReportordersmodel extends MY_Model {
  ...
  function total_count()
  {
    $this->db->select("1");
      ...
      /*^^^^^ Get WorkflowQueueReport Orders Query ^^^^^*/
      $this->Common_Model->GetWorkflowQueueReportQueueOrders();
...
$this->Common_Model->FilterByProjectUser($this->RoleUID,$this->loggedid);     
  ...
  function WorkflowQueueReportOrders($post,$global='') {

      // dynamic column common query
      $post['Section'] = $this->uri->segment(1);
      $this->Common_Model->DynamicColumnsCommonQuery('', TRUE, $post);
    ...
    function GetWorkflowQueueReportOrdersExcelRecords($post)
    {
      $this->Common_Model->DynamicColumnsCommonQuery($WorkflowModuleUID = '',TRUE);
