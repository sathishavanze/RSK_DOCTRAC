...
class Scheduling_Orders_Model extends MY_Model {
	...
	function total_count()
	{
		...
		/*^^^^^ Get MyOrders Query ^^^^^*/
		$this->Common_Model->GetSchedulingQueue(false);
  ...
  // New Orders
	function count_all()
	{
		...
		$this->Common_Model->GetSchedulingQueue();
      ...
      // Datatable OrderBy
			...
			function SchedulingOrders($post,$module,$global='')
			{
				$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
  ...
  // Work in Progress
      ...
      // Datatable OrderBy
			...
			function ThreeAConfirmationOrders($post,$module,$global='')
			{
				$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
				...
				// Email and Phone 
				$this->db->select('tOrderWorkflowsData.IsPhoneEnabled,tOrderWorkflowsData.IsEmailEnabled');

				/*^^^^^ Get MyOrders Query ^^^^^*/
				$this->Common_Model->GetSchedulingQueue(FALSE, ['ThreeAConfirmationSigningDateCond'=>TRUE]);
				
				// 3A Confirmation Conditions
				$this->ThreeAConfirmationConditions();
			...
			// New Orders
			function ThreeAConfirmationcount_all($post = FALSE)
			{
				$this->db->select("1");
			...
			function ThreeAConfirmationcount_filtered($post,$module)
			{
				if (!empty($post['search_value']) || (!empty($post['order']) && isset($post['column_order'][$post['order'][0]['column']]) && !empty($post['column_order'][$post['order'][0]['column']]))) {
					$this->Common_Model->DynamicColumnsCommonQuery($this->WorkflowModuleUID);
				}
			...
			public function ThreeAConfirmationConditions()
			{
				// Join tOrderWorkflowsData table
				$this->db->join('tOrderWorkflowsData','tOrderWorkflowsData.OrderUID = tOrders.OrderUID','left');
			}

			//update phone
			function UpdatePhone($OrderUID, $WorkflowModuleUID, $IsPhoneEnabled) {
				$where = array('OrderUID' => $OrderUID, 'WorkflowModuleUID' => $WorkflowModuleUID);
				$query = $this->db->get_where('tOrderWorkflowsData', $where);
        		if ($query->num_rows() > 0) {
        			$this->db->where($where);
        			return $this->db->update('tOrderWorkflowsData', array('IsPhoneEnabled' => $IsPhoneEnabled));
        		}	
        		return $this->db->insert('tOrderWorkflowsData', array('OrderUID' => $OrderUID, 'WorkflowModuleUID' => $WorkflowModuleUID, 'IsPhoneEnabled' => $IsPhoneEnabled));
			}

			//update Email
			function UpdateEmail($OrderUID, $WorkflowModuleUID, $IsEmailEnabled) {
				$where = array('OrderUID' => $OrderUID, 'WorkflowModuleUID' => $WorkflowModuleUID);
				$query = $this->db->get_where('tOrderWorkflowsData', $where);
        		if ($query->num_rows() > 0) {
        			$this->db->where($where);
        			return $this->db->update('tOrderWorkflowsData', array('IsEmailEnabled' => $IsEmailEnabled));
        		}	
        		return $this->db->insert('tOrderWorkflowsData', array('OrderUID' => $OrderUID, 'WorkflowModuleUID' => $WorkflowModuleUID, 'IsEmailEnabled' => $IsEmailEnabled));
			}
