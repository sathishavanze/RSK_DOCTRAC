...
class Scheduling_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('Scheduling_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['Scheduling'];
		$this->WorkflowModuleName = 'Scheduling';
	} 
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->Scheduling_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['Scheduling']);
    ...
    //column search
        ...
        */
		...
		$list = $this->Scheduling_Orders_Model->SchedulingOrders($post,'');
		...
		/**
			...
			*/
		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] 		= "Scheduling/index/";
		$Mischallenous['AssignButtonClass'] = "SchedulingPickNewOrder";
		...
		{
			$Schedulinglist 				= 	$DynamicColumns['orderslist'];
		...
		foreach ($list as $myorders)
		{
			...
			{
				$Action = '<a href="Scheduling/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{
				$Action = '<button class="btn btn-info btn-sm SchedulingPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = '<input type="text" data-OrderUID="'.$myorders->OrderUID.'" class="schedule_date" value="">';
			$row[] = '<input type="text" data-OrderUID="'.$myorders->OrderUID.'" class="schedule_time" value="">';
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Scheduling_Orders_Model->count_all(),
			"recordsFiltered" =>  $this->Scheduling_Orders_Model->count_filtered($post,''),
	...
	function workinginprogress_ajax_list()
	{
		...
		$post['order'] = $this->input->post('order');
		$post['draw'] = $this->input->post('draw');
		//if(isset($this->input->post('ReportType'))){
  	...
  	//column order
        ...
        */
        ...
        //column order
		$list = $this->Scheduling_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm SchedulingPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Scheduling_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->Scheduling_Orders_Model->inprogress_count_filtered($post),
    ...
    //column search
        ...
        */
		...
		$list = $this->Scheduling_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Scheduling_Orders_Model->myorders_count_all(),
			"recordsFiltered" =>  $this->Scheduling_Orders_Model->myorders_count_filtered($post),
    ...
    //column search
        ...
        */
		...
		$list = $this->Scheduling_Orders_Model->parkingorders($post,'');
		$no = $post['start'];
		$myorderslist = [];
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				$Action = '<a href="Scheduling/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs SchedulingPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}	
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Scheduling_Orders_Model->parkingorders_count_all(),
			"recordsFiltered" =>  $this->Scheduling_Orders_Model->parkingorders_count_filtered($post),
	...
	function WriteExcel()
	{
		...
		{
			$list1 = $this->Scheduling_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list2 = $this->Scheduling_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->Scheduling_Orders_Model->parkingorders($post,'');
	...
	/**
	*Function Description: Add date with hardstop working days for schedule date update
	*Used in checkSchedule function 
	...
	*/
    ...
    /**
	*Function Description: Check schedule date updating conditions and show warning
	*Used in updateSchedule function 
	*@author Santhiya M <santhiya.m@avanzegroup.com>
	*@since Friday 10 July 2020
	*Modified at Monday 13 July 2020
	*/
	function checkSchedule(){
		...
		$hardStop = $this->Common_Model->pluckSettingValue('HardStop');
		/* Des: Motified to avoid weekends in hardstop condition */	
		$hardStoptime = $this->AddDays(strtotime(date('Y-m-d H:i:s')), $hardStop, $skipdays,$skipdates = array());
		$hardStopDiff = date('Y-m-d H:i:s', $hardStoptime);

		$tOrders = $this->db->select('EarliestClosingDate')->from('tOrderImport')->where('OrderUID', $OrderUID)->get()->row();
		if($hardStopDiff <= $combinedDT && (strtotime($tOrders->EarliestClosingDate) <= strtotime($schedule))){
			$res = array('status'=>0, "Warn"=>'Write Schedule');
		}else if(strtotime($tOrders->EarliestClosingDate) >= strtotime($schedule)){
			$warn = 'Scheduled Date should be equal or after Earliest Closing date of '.site_datetimeaging($tOrders->EarliestClosingDate).'.  Do you want to update anyway?';
			$res = array('status'=>1, "Warn"=>$warn);
		}elseif($hardStopDiff > $combinedDT){
			$warn = 'Scheduled Date should be '.$hardStop.' hours after from now! Do you want to update anyway?';
			$res = array('status'=>1, "Warn"=>$warn);
		}else{
			$res = array('status'=>1, "Warn"=>'none');
		}

		echo json_encode($res);	
	}
	/**
	*Function Description: Update schedule date & time
	*@author Santhiya M <santhiya.m@avanzegroup.com>
	*@since Friday 9 July 2020
	*/
	function updateSchedule(){
		...
		$date = date_create($combinedDT);

		date_time_set($date, -$hardStop, 0);
		$hardStopDiff = date_format($date, 'Y-m-d H:i:s');

		if($this->Common_Model->ScheduleUpdate($this->input->post())){
			$res = array('status'=>0, "Warn"=>'Schedule Updated');			
		}else{
			$res = array('status'=>1, "Warn"=>'Schedule Updated failed');
		}
		echo json_encode($res);
	}

	/**
	*Function 3A Confirmation 
	*@author SathishKumar <sathish.kumar@avanzegroup.com>
	*@since Thursday 03 September 2020.
	*/
	function ThreeAConfirmationOrdersAjaxList(){
		//Advanced Search
		$post['advancedsearch'] = $this->input->post('formData');
		//Advanced Search
		//get_post_input_data
        ...
        */
		...
		$list = $this->Scheduling_Orders_Model->ThreeAConfirmationOrders($post,'');
		$no = $post['start'];
		$Schedulinglist = [];
		...
		*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->Scheduling_Orders_Model->ThreeAConfirmationcount_all($post),
			"recordsFiltered" =>  $this->Scheduling_Orders_Model->ThreeAConfirmationcount_filtered($post,''),
			"data" => $data['orderslist'],
		);
	...
	function UpdatePhone()
	{
		...
		$IsPhoneEnabled = $this->input->post('IsPhoneEnabled');

		$result = $this->Scheduling_Orders_Model->UpdatePhone($OrderUID, $WorkflowModuleUID, $IsPhoneEnabled);
		if($result) {
	...
	function UpdateEmail()
	{
		$OrderUID = $this->input->post('OrderUID');
		$WorkflowModuleUID = $this->input->post('WorkflowModuleUID');
		$IsEmailEnabled = $this->input->post('IsEmailEnabled');

		$result = $this->Scheduling_Orders_Model->UpdateEmail($OrderUID, $WorkflowModuleUID, $IsEmailEnabled);
		if($result) {
			echo json_encode(array('error'=>0,'msg'=>'Updated.','type'=>'success'));
		} else {
			echo json_encode(array('error'=>0,'msg'=>'Something went wrong!.','type'=>'danger'));
