<?php
/**
* Module to manage the files from SFTP server for an Order 
* sftp folder
* @param OrderUID
*
* @throws no exception
...
* @since 15th July 2020
*
*/
class SFTP_Files extends MX_Controller
{
	public $pfilename;
	private $_lock;
	...
	public function index()
	{
		...
		$data['SFTP_Files'] = $this->Common_Model->GetSftpFiles($OrderUID);
		// echo '<pre>';print_r($data['SFTP_Files']);exit;
	...
	public function importPDFLoans()
	{
		...
		foreach ($mAutoImport as $key => $value) 
		{
			...
			foreach ($waitingLoans as $loankey => $loanvalue) {
				...
				echo "pickup loan -".$loanvalue->LoanNumber;
				foreach ($list as $listkey => $listvalue) {
					if($listvalue== $loanvalue->$selectedColumnName.'.pdf')
					{
						...
						$data = array(
							...
							'IsStacking'		=>	'1',
							'TypeofDocument'	=>	'SFTP_Files',
							'UploadedDateTime'	=>	date('Y-m-d H:i:s')
						);
						$this->Common_Model->save('tDocuments',$data);

						/**
						*Upload file to completed folder also 
						*@author Santhiya M <santhiya.m@avanzegroup.com>
						*@since Thursday July 16 2020
						*/	
						if(file_exists(FCPATH.$completePath)){
							$CompletedDocumentPath = FCPATH . 'uploads/OrderDocumentPath/CompletedOrders/'.$loanvalue->OrderNumber;
							$this->Common_Model->CreateDirectoryToPath($CompletedDocumentPath);
							if( file_put_contents($CompletedDocumentPath . '/' . $loanvalue->LoanNumber.'.pdf', file_get_contents(FCPATH.$completePath)) ) {
								$WorkflowModuleUID = $this->config->item('Workflows')['HOI'];
								$HoiComplete = $this->Common_Model->CompleteHoi($loanvalue->OrderUID, $WorkflowModuleUID);
							}								
						}					
					}
					if($count >= $limit)
