...
$this->Common_Model->FilterByProjectUser($this->RoleUID,$this->loggedid);
  ...
  function MyOrders($post,$module)
  {

    
    $this->db->select("tOrders.*, tOrderPackage.PackageNumber, mProjectCustomer.ProjectName, tOrders.OrderUID,tOrders.LoanNumber, mStatus.StatusName, mStatus.StatusColor ,mCustomer.CustomerName, mProjectCustomer.ProjectUID, mProducts.ProductName, tOrderAssignments.AssignedToUserUID");
...
$this->Common_Model->FilterByProjectUser($this->RoleUID,$this->loggedid);
  ...
  function AssignOrders($id,$ProjectUID,$Workflow)
  {
    ...
    function GetMyOrdersExcelRecords($post)
    {
      ...
      $this->db->select("*,tOrders.OrderUID,tOrders.LoanNumber,mStatus.StatusName,mStatus.StatusColor,mCustomer.CustomerName,mProjectCustomer.ProjectUID,mProducts.ProductName");
      $this->db->select("DATE_FORMAT(tOrders.OrderEntryDatetime, '%m-%d-%Y %H:%i:%s') as OrderEntryDatetime", FALSE);
