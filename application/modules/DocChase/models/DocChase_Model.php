...
class DocChase_Model extends MY_Model { 
	function __construct()
	{ 
		parent::__construct();
		$this->lang->load('keywords');
		$this->WorkflowUID = $this->config->item('Workflows')['DocChase'];
	}
	...
	function get_docchase_workflows($OrderUID)
	{
		return $this->db->select('*')->from('mWorkFlowModules')->join('tOrderDocChase','tOrderDocChase.WorkflowModuleUID=mWorkFlowModules.WorkflowModuleUID')->where(array('OrderUID'=>$OrderUID,'IsCleared'=>0))->group_by('tOrderDocChase.WorkflowModuleUID')->get()->result();
	}
