...
<!--     <div class="row">
  ...
  <div class="card-body">
    <ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#Canceled" role="tablist">
          Cancelled Orders
          <span class="badge badge-pill badge-primary" id="Canceled-count" style="background-color: #fff;color: #000;"><?php echo $this->Canceledordersmodel->count_all(); ?></span>
    ...
    <div  class="material-datatables" >
      ...
      <table class="table table-hover table-striped" id="Canceled">
        <thead>
         <tr>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    Canceledinitialize('false')
    function Canceledinitialize(formdata)
    {
      Canceled = $('#Canceled').DataTable( {
        scrollX:        true,
        scrollCollapse: true,
        fixedHeader: false,
        scrollY: '100vh',
        ...
        "autoWidth": true,
          ...
          "ajax": {
            "url": "<?php echo base_url('Canceled/Canceledorders_ajax_list')?>",
            "type": "POST",
...
$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
  ...
  $.ajax({
   type: "POST",
   url: '<?php echo base_url();?>Canceled/WriteExcel',
   xhrFields: {
    responseType: 'blob',
  },
  data: {'formData':formData},
...
$(document).off('click', '.ExportDocument').on('click', '.ExportDocument', function (e) {  
  ...
  SwalConfirmExport(OrderUID, OrderNumber, currentrow, Canceled, LoanNumber);
})
