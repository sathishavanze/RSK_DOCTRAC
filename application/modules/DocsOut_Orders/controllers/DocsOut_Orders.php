...
class DocsOut_Orders extends MY_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('DocsOut_Orders_Model');
		$this->WorkflowModuleUID = $this->config->item('Workflows')['DocsOut'];
		$this->WorkflowModuleName = 'DocsOut';
	} 
	...
	public function index()
	{
		...
		$data['IsGetNextOrder'] = $this->DocsOut_Orders_Model->CheckAutoAssignEnabled($this->loggedid);
		...
		$data['IsParking'] = $this->Common_Model->is_parking_enabledforworkflow($this->config->item('Workflows')['DocsOut']);
		$data['IsKickBack'] = $this->Common_Model->is_kickback_enabledforworkflow($this->config->item('Workflows')['DocsOut']);
	...
	function neworders_ajax_list(){
  	...
  	//get_post_input_data
  	//column order
		$post['column_order'] = array('tOrders.LoanNumber', 'tOrderPropertyRole.BorrowerFirstName','mMilestone.MilestoneName','tOrderImport.LoanProcessor','tOrders.LoanType','tOrders.PropertyStateCode', 'tOrderImport.DocsOutSigningDate', 'tOrderImport.DocsOutSigningTime','tOrderImport.QueueDate','tOrderImport.Funder','Earliest Closing Date','Closing Disclosure Send Date','tOrderImport.LockExpiration','tOrderImport.OccupancyStatus','tOrderImport.CashFromBorrower');
    ...
    //column search
        ...
        */
		...
		$list = $this->DocsOut_Orders_Model->DocsOutOrders($post,'');
		...
		/**
			...
			*/
		/* ****** Dynamic Queues Section Starts ****** */
		$Mischallenous['PageBaseLink'] 		= "DocsOut/index/";
		$Mischallenous['AssignButtonClass'] = "DocsOutPickNewOrder";
		...
		{
			$DocsOutlist 				= 	$DynamicColumns['orderslist'];
		...
		{ 
			...
			$row[] = site_datetimeaging($myorders->DocsOutSigningTime);
			$row[] = site_datetimeaging($myorders->QueueDate);
			$row[] = $myorders->Funder;
			$row[] = site_datetimeaging($myorders->EarliestClosingDate);
			$row[] = site_datetimeaging($myorders->DocsOutClosingDisclosureSendDate);
			...
			{
				$Action = '<a href="DocsOut/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-info btn-sm DocsOutPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="fa fa-hand-o-up" aria-hidden="true"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
			$DocsOutlist[] = $row;
		}
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->count_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->count_filtered($post,''),
	...
	function docschecked_ajax_list(){
		//Advanced Search
		$post['advancedsearch'] = $this->input->post('formData');
		//Advanced Search
		//get_post_input_data
    ...
    //column search
        ...
        */
		...
		$list = $this->DocsOut_Orders_Model->DocsCheckOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->Docs_count_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->Docs_count_filtered($post,''),
			"data" => $data['orderslist'],
		);
	...
	function queuechecked_ajax_list(){
		//Advanced Search
		$post['advancedsearch'] = $this->input->post('formData');
		//Advanced Search
		//get_post_input_data
    ...
    //column search
        ...
        */
		...
		$list = $this->DocsOut_Orders_Model->QueuecheckOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->Queuecheck_count_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->Queuecheck_count_filtered($post,''),
			"data" => $data['orderslist'],
		);
...
function pendingdocs_ajax_list(){
		//Advanced Search
		$post['advancedsearch'] = $this->input->post('formData');
		//Advanced Search
		//get_post_input_data
    ...
    //column search
        ...
        */
		...
		$list = $this->DocsOut_Orders_Model->PendingdocsOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->Pendingdocs_count_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->Pendingdocs_count_filtered($post,''),
			"data" => $data['orderslist'],
		);
    ...
    //column search
        ...
        */
		...
		$list = $this->DocsOut_Orders_Model->PendinguwOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->Pendinguw_count_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->Pendinguw_count_filtered($post,''),
			"data" => $data['orderslist'],
		);
    ...
    //column search
        ...
        */
        ...
        //column order
		$list = $this->DocsOut_Orders_Model->WorkInProgressOrders($post,'');
		...
		foreach ($list as $myorders)
		{
			...
			// $row[] = '<a  href="javascript:void(0)" style=" background: '.$myorders->StatusColor.' !important;padding: 5px 10px;border-radius:0px;" class="btn">'.$myorders->StatusName.'</a>';
			$row[] = $myorders->PropertyStateCode;
			$row[] = site_datetimeaging($myorders->DocsOutSigningDate);
			$row[] = site_datetimeaging($myorders->DocsOutSigningTime);
			$row[] = site_datetimeaging($myorders->QueueDate);
			$row[] = $myorders->Funder;
			$row[] = site_datetimeaging($myorders->EarliestClosingDate);
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				...
				$Action = '<button class="btn btn-info btn-sm DocsOutPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}
			$row[] = $Action;
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->inprogress_count_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->inprogress_count_filtered($post),
  ...
  //column order
    ...
    //column search
        ...
        */
		...
		$list = $this->DocsOut_Orders_Model->MyOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->myorders_count_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->myorders_count_filtered($post),
   ...
   //column order
    ...
    //column search
		...
		$list = $this->DocsOut_Orders_Model->parkingorders($post,'');
		$no = $post['start'];
		$myorderslist = [];
		...
		foreach ($list as $myorders)
		{
			...
			if($this->loggedid == $myorders->AssignedToUserUID)
			{
				$Action = '<a href="DocsOut/index/'. $myorders->OrderUID.'" class="btn btn-link btn-info btn-just-icon btn-xs" data-orderuid = "'.$myorders->OrderUID.'" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</a>';
			}else{

				$Action = '<button class="btn btn-link btn-info btn-just-icon btn-xs DocsOutPickNewOrder" data-workflowmoduleuid="'.$myorders->WorkflowModuleUID.'" data-orderuid="'.$myorders->OrderUID.'" data-projectuid="'.$myorders->ProjectUID.'"><i class="icon-pencil"></i>'.$FollowUp.'</button>';
			}	
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->parkingorders_count_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->parkingorders_count_filtered($post),
 ...
 //column order
    ...
    //column search
		...
		$list = $this->DocsOut_Orders_Model->KickBackDocsOutOrders($post,'');
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->KickBackcount_all($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->KickBackcount_filtered($post,''),
			"data" => $data['orderslist'],
		);
	...
	function WriteExcel()
	{
		...
		if($post['formData']['Status'] == 'orderslist')
		{
		   ...
		   // $data[] = array($list[$i]->LoanNumber,$list[$i]->BorrowerFirstName,$list[$i]->MilestoneName,$list[$i]->LoanType,$list[$i]->LoanProcessor,$list[$i]->PropertyStateCode,site_datetimeaging($list[$i]->DocsOutSigningDate),site_datetimeformat($list[$i]->DocsOutSigningTime),site_datetimeformat($list[$i]->QueueDate),$list[$i]->Funder,site_datetimeformat($list[$i]->EarliestClosingDate),site_datetimeformat($list[$i]->EarliestClosingDate),$list[$i]->LockExpiration,$list[$i]->OccupancyStatus,$list[$i]->CashFromBorrower);				
		...
		{
			$list1 = $this->DocsOut_Orders_Model->WorkInProgressOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
		...
		{
			$list2 = $this->DocsOut_Orders_Model->MyOrders($post,'');
			/**
			*Function Description: Separate EXCEL SHEET
			*@author Shruti <shruti.vs@avanzegroup.com>
			*@since Date
		...
		{
			$list3 = $this->DocsOut_Orders_Model->parkingorders($post,'');
	...
	function SubmittedforDocCheck_AjaxList(){
		//Advanced Search
		$post['advancedsearch'] = $this->input->post('formData');
		//Advanced Search
        ...
        */
		...
		$list = $this->DocsOut_Orders_Model->SubmittedforDocCheckOrders($post,'');
		...
		*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->SubmittedforDocCheck_CountAll($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->SubmittedforDocCheck_CountFiltered($post,''),
			"data" => $data['orderslist'],
		);
	...
	function NonWorkable_AjaxList(){
        ...
        */
		...
		$list = $this->DocsOut_Orders_Model->NonWorkableOrders($post,'');
		...
		*/
		...
		$output = array(
			"draw" => $post['draw'],
			"recordsTotal" => $this->DocsOut_Orders_Model->NonWorkable_CountAll($post),
			"recordsFiltered" =>  $this->DocsOut_Orders_Model->NonWorkable_CountFiltered($post,''),
			"data" => $data['orderslist'],
		);
	...
	/**
	*Function Fet Widget Count 
	*@author SathishKumar <sathish.kumar@avanzegroup.com>
	*@since Saturday 31 October 2020.
	*/
	function GetSubQueueWidgetCounts() {
		
		$SubQueueWidgetCounts = [];

		// Gatekeeping Filter List
		$filterlist = $this->input->post('filterlist');
		$QueueUID = $this->input->post('QueueUID');
    	$post['advancedsearch']['WorkflowModuleUID'] = $this->WorkflowModuleUID;
    	
		if ($filterlist == '#orderslist') {
			$post['advancedsearch']['IsTATMissedNewOrdersFilter'] = 'true';
			$SubQueueWidgetCounts['DocsOutTATMissedNewOrdersCount'] = $this->DocsOut_Orders_Model->count_all($post);
		} elseif ($filterlist == '#pendinguworderslist') {
			$post['advancedsearch']['IsTATMissedPendingFromUWFilter'] = 'true';
			$SubQueueWidgetCounts['DocsOutTATMissedPendingFromUWCount'] = $this->DocsOut_Orders_Model->Pendinguw_count_all($post);
		}elseif ($filterlist == '#docsorderslist') {
			$post['advancedsearch']['IsDocsCheckedConditionPendingFollowupPastDueFilter'] = 'true';
			$SubQueueWidgetCounts['DocsOutDocsCheckedConditionPendingFollowupPastDueCount'] = $this->DocsOut_Orders_Model->Docs_count_all($post);
			unset($post['advancedsearch']['IsDocsCheckedConditionPendingFollowupPastDueFilter']);
			$post['advancedsearch']['IsDocsCheckedConditionPendingFollowupYetToBeReviewedFilter'] = 'true';
			$SubQueueWidgetCounts['DocsOutDocsCheckedConditionPendingFollowupYetToBeReviewedCount'] = $this->DocsOut_Orders_Model->Docs_count_all($post);
		}

		echo json_encode($SubQueueWidgetCounts);	
