...
<div class="card mt-20" id="Exceptionorders">
    ...
    <div class="row">
      <div class="col-md-6">
        <h4 class="card-title">Exception Report</h4>
      </div>
    </div>
  </div>
  ...
  <div class="card-body">
    <ul class="nav nav-pills nav-pills-rose customtab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#DailyExceptionReport" role="tablist">
          Exception Report
          <span class="badge badge-pill badge-primary" id="completed-count" style="background-color: #fff;color: #000;"><?php echo $this->DailyExceptionmodel->count_all(); ?></span>
    ...
    <?php //$this->load->view('common/advancesearch'); ?>
        ...
        <table class="table table-hover table-striped" id="DailyExceptionReport">
            <thead>
               <tr>
                  <th>Order Number</th>
                  <th>Loan Number</th>
...
<script type="text/javascript">
          var DailyExceptionReport = false;
        $(function() {
          $("select.select2picker").select2({
            //tags: false,
            theme: "bootstrap",
          });
          $('#DailyExceptionReport').DataTable().destroy();
        });
...
dailyexceptionreport('false');
    function dailyexceptionreport(formdata)
    {
          DailyExceptionReport = $('#DailyExceptionReport').DataTable( {
            scrollX:        true,
            scrollCollapse: true,
            fixedHeader: false,
            scrollY: '100vh',
          ...
          "ajax": {
            "url": "<?php echo base_url('DailyException/FetchDataException')?>",
            "type": "POST",
...
$(document).off('click','.exceldownload').on('click','.exceldownload',function(){
  ...
  $.ajax({
     type: "POST",
     url: '<?php echo base_url();?>DailyException/WriteExcel',
          xhrFields: {
      responseType: 'blob',
    },
     data: {'formData':formData},
    ...
    {
        var filename = "DailyExceptionOrders.csv";
        if (typeof window.chrome !== 'undefined') {
            //Chrome version
            var link = document.createElement('a');
            link.href = window.URL.createObjectURL(data);
            link.download = "DailyExceptionOrders.csv";
            link.click();
...
$(document).off('click', '.ExportDocument').on('click', '.ExportDocument', function (e) {  
  ...
  SwalConfirmExport(OrderUID, OrderNumber, currentrow, DailyExceptionReport, LoanNumber);
})
