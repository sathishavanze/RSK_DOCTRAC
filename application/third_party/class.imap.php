...
/*    public function __construct()
    ...
    public function __construct(array $config = [])
    {
        if (! empty($config))
        {
            foreach ($config as $key => $value) {
                if ($key == "attachments_dir") {
                    $this->attachments_dir = $value; 
                }
            }
            //$this->connect();
        }
        ...
        if (!file_exists($this->attachments_dir)) {
            mkdir($this->attachments_dir, 0777, true);
        }
    }
    ...
    public function getMessages($type = 'text', $sort = 'asc', $filter = "ALL")
    {
        $stream = $this->imapStream;
        $this->emails = imap_search($stream, $filter);
