...
function tessaract($allData){	
	...
	if (file_exists($filepath)){ 
		...
		if (!is_dir(FILEPATH.$allData['orderNo']."/text")) {	
			mkdir(FILEPATH.$allData['orderNo']."/text", 0777, true);
		}
		...
		if ($allData["docType"] == 'pdf' || $allData["docType"] == 'PDF') {
			...
			$resultData=getSplitPdfResult($allData);			

		}else{
			$resultData=getImageResult($allData);	
		}
...
function getSplitPdfResult($allData){
	...
	$filename=FILEPATH.$allData['orderNo']."/text/".$split_name[0];
	if(OCR){
		for ($i=1; $i <=$count ; $i++) {	
			if (!file_exists(FILEPATH.$allData['orderNo']."/text/".$split_name[0]."-".$i.".txt")){		
			LOG_MSG('INFO','getSplittedPdfResult(): tesseractInstance START  \n'); 		 	 		
			$tesseractInstance = new TesseractOCR(FILEPATH.$allData['orderNo'].'/images/"'.$split_name[0]."-".$i.'.jpg"');
			$ocrResult = $tesseractInstance->run();
			LOG_MSG('INFO','getSplittedPdfResult(): tesseractInstance END \n');			
			$fp = fopen(FILEPATH.$allData['orderNo']."/text/".$split_name[0]."-".$i.'.txt', 'w');

			$cmd = "sudo chown -R www-data:www-data ".FILEPATH.$allData['orderNo']."/text/".$split_name[0]."-".$i.".txt";
			$res = exec($cmd);
			$cmd = "sudo chmod -R 777 ".FILEPATH.$allData['orderNo']."/text/".$split_name[0]."-".$i.".txt";
			$res = exec($cmd);

			//chmod(FILEPATH.$allData['orderNo']."/text/".$split_name[0]."-".$i.".txt", 0777); 
			//chown(FILEPATH.$allData['orderNo']."/text/".$split_name[0]."-".$i.".txt", 'www-data');
			fwrite($fp,$ocrResult);
			fclose($fp);
			$path=FILEPATH.$allData['orderNo']."/text/".$split_name[0]."-".$i.'.txt';
	...
	if(CLASSIFICATION){
		for ($m=1; $m <=$count ; $m++) { 
			$results = checkConfidence(FILEPATH.$allData['orderNo']."/text/".$split_name[0]."-".$m.".txt",$allData, $m,$filename);

			// if(DEBUG): print_r($results); endif;
		}
	}
	...
	if(DEBUG) print_r($resultData);echo "\n";
	return $resultData;
}
...
function getImageResult($allData){
	...
	$fp = fopen(FILEPATH.$allData['orderNo']."/text/".$split_name[0].'.txt', 'w');
	chmod(FILEPATH.$allData['orderNo']."/text/".$split_name[0].".txt", 0777); 
	chown(FILEPATH.$allData['orderNo']."/text/".$split_name[0].".txt", 'www-data');     
	fwrite($fp, json_encode($result));
	fclose($fp);
	$filename=FILEPATH.$allData['orderNo']."/text/".$split_name[0];
	checkConfidence(FILEPATH.$allData['orderNo']."/text/".$split_name[0].".txt",$allData['docDef'], $pageNo,$filename);				
		...
		if ($key == 'Count') {
			...
			foreach( $value as $k => $v) { 
				...
				if ($k == 'MandatoryKeywords') {
					...
					$getPageContent=getPageContent($ocrText,$Header,$Footer,$LowerCaseSearch);
					...
					if($pageConfidence >= $Confidence){
						...
						if(isset($resultData[$pageNo])){
							...
							if(SUBCLASSIFICATION){
								$pageSubData = checkAgencyConfidence($txtFile,$data['subDocDef'],$pageNo,$filename);
							...
							if(preg_match($pattern, $tempHeader.$tempFooter, $matches)){
								$string=$matches[0];
								getUnclassifiedPages($string,$filename,$pageNo,$key,$docTypeUID,$tempHeader.$tempFooter,$pattern,$Header,$Footer,$LowerCaseSearch);
							}elseif (preg_match($pattern1, $tempHeader.$tempFooter, $matches)) {
								$string=$matches[0];
								getUnclassifiedPages($string,$filename,$pageNo,$key,$docTypeUID,$tempHeader.$tempFooter,$pattern1,$Header,$Footer,$LowerCaseSearch);
							}
						}
					}
				}
...
function checkAgencyConfidence($txtFile,$data,$pageNo,$filename){
	...
	// print_r('Agency pageNo:' .$pageNo. "\n");
	// print_r('resultData');
	// print_r($resultData);
	// print_r('Count : '. count($resultData)."\n");
	...
	foreach ($fdata as $key => $value){ 
		if ($key == 'Count') {
			...
			// if(DEBUG) print_r('before Agency ManKeyCount :' .$ManKeyCount. "\n");
			// print_r('key :' .$key. "\n");
			...
			foreach( $value as $k => $v) { 
				...
				// print_r('subConfidence :' .$subConfidence. "\n");
				...
				if($ManKeyCount && $k == 'Keywords'){ 
					...
					// if(DEBUG) print_r('Agency KeyCount:' .$KeyCount. "\n");
	
					$pageSubConfidence =(($KeyWordCutOff != 0)? 0.5 + (($KeyCount/$KeyWordCutOff)/2):0.5 + (($KeyCount/$KeywordsCount)/2));

					// print_r('Agency pageSubConfidence:' .$pageSubConfidence. "\n");
					if($pageSubConfidence >= $subConfidence){

						// print_r('satisfied on key ' .$key. "\n");

						// $k = count($resultData[$pageNo]);

						// for($si=0;$si<$k;$si++){

						// 	$resultData[$pageNo][$si]['subDocName']=$key;
						// 	$resultData[$pageNo][$si]['subDocTypeUID']=$subDocTypeUID;
						// 	$resultData[$pageNo][$si]['pageSubConfidence']=$pageSubConfidence;
						// }
	...
	// print_r("returnArray \n");
	// echo '<pre> Response: ';print_r($returnArray); 
	...
	for($i=$startPage, $n=1 ;$i<=$endPage;$i++,$n++){
		...
		if(stripos($tempHeader.$tempFooter,$pagePattern)){
			...
			if(!isset($resultData[$i])){
				...
				if (EXTRACTION){
					$PageArray['extractedData']=[];
					$extractedData=DataExtraction($ocrText,$key);
					array_push($PageArray['extractedData'],$extractedData);
				}
		...
		if ($InsuranceAgency == "Western Mutual Insurance"){
			...
			for ($t=1; $t <= $template; $t++) {
				...
				if($t == 1){
					...
					if(preg_match($pattern, $contents, $matches)){
						$text = trim($matches[1]);
						$tmpData[$t]['Website'] ="www."."". $matches[1];
					}else{if(DEBUG) echo "No matches found";}
					...
					if(preg_match($pattern, $contents, $matches)){
						$text = trim($matches[3]);
						$tmpData[$t]['Dwelling Amount'] = $matches[3];
					}else{if(DEBUG) echo "No matches found";}
